"""
Eltec 449 M18 ESP32 tester - TP443 frequency-tracking build (based on the
405 M22 / v6.1 testers).

This build adapts the unified rig tester to the Model 449 M18 pyroelectric
detector, following document TP443 page 6 "449M18 Frequency Tracking". The
legacy fixture (9000142 + optical bench, 500 K blackbody, 0.873" aperture,
10 cm, -25 filter) chops the source with a MECHANICAL 20/80 blade at 5 Hz and
then at 18 Hz and reads the sensitivity on a scope at each frequency:

    Spec 1  sensitivity at  5 Hz  >= 1.2 V   (-25 filter)
    Spec 2  sensitivity at 18 Hz  >= 0.72 V  (-25 filter)
    Spec 3  ratio 18 Hz / 5 Hz    0.70 ... 1.30
    Spec 4  sampling: one sample failure, or a ratio of 0.72 or less, means the
            whole tray is measured 100 %. Only parts failing specs 1-3 fail.

This rig has no blade: the ESP32 PULSES the IR emitter electrically at
5 Hz / 20 % duty and then 18 Hz / 20 % duty (firmware v3.2 adds PWM,DUTY),
and reads the part's pk-pk response at each frequency on ADS1256 AIN0. An
electrically pulsed emitter does NOT modulate its output equally at 5 Hz and
18 Hz (the thermal swing per pulse shrinks with frequency), so the raw
readings are NOT comparable to the legacy scope values and the raw ratio is
NOT the TP443 ratio. Each frequency therefore gets its OWN fixture
calibration factor (legacy-equivalent = raw x K_f, derived from a paired
comparison of the same parts on both fixtures, exactly like the 405 M22's
lot-500 factor) and the corrected ratio K18*N18 / (K5*N5) recovers the
legacy ratio. Until those factors exist (SENSITIVITY_GATE_ENABLED is False)
the app records the raw readings and the raw ratio, applies only the
"is there a stable positive signal" checks, and marks every verdict
CALIBRATION PENDING.

The hardware backend is the same ESP32 + ADS1256 rig and requires firmware
v3.2 (PWM,DUTY for the 20/80 drive; the gain-1/unbuffered front end).
...
Wiring / power (batteries isolated 2026-08-12 to fix the emitter spike):
    ...
    ADS AIN1 = permanently-mounted reference sensor (emitter-health gate,
               live again since 2026-09-09 on the channel-isolated buffer
               board, see REFERENCE_GATE_ENABLED)
    ADS AIN7 = legacy battery divider tap - NOT MONITORED anymore (see below)
    GPIO33   = PWM output to the MOSFET module (20 % ON / 80 % OFF for the DUT)
...
Model 449 M18 notes (TP443):
    - The procedure measures ALL parts at 5 Hz first and then all at 18 Hz,
      because the blade speed is changed by hand. Here the drive switches
      electronically, so each part gets both captures back to back and its
      ratio on the spot (recorded per frequency in the batch CSV either way).
    - TP443's offset/noise pages were not available when this build was
      written: the DC offset is read and recorded (and a near-0 V input is
      still treated as "no sensor"), but the offset band is NOT gated until
      OFFSET_GATE_ENABLED is turned on with the real TP443 limits. There is
      no emitter-off noise test in this build.
    - Sync at 18 Hz: at 1000 SPS one 18 Hz period spans 55.56 samples, so
      cycle boundaries are quantized to whole samples; sync validation
      judges the mean cadence against TP443's +/-0.18 Hz and gives single
      cycles a one-sample allowance (see stability_analysis).

Guided flow:
    1. Enter the batch details (the TP443 filter setup is -25).
    2. Place the sensor in the rig and press Enter.
    3. The app runs the per-part steps with a step progress bar:
       (a) DC offset read, emitter off (plausibility + record);
       (b) 5 Hz / 20 % drive - adaptive peak stability, then a 20-cycle
           pk-pk measurement (spec 1 once calibrated);
       (c) 18 Hz / 20 % drive - same rule, 36-cycle window (spec 2 once
           calibrated); then the 18/5 ratio (spec 3) and the tray flag
           (spec 4) are evaluated and shown.

Run:
    python3 eltec_449m18_esp32_tester.py
"""
...
# ---- Model 449 M18 overrides of the shared 406MCA engine ------------------- #
# The shared v1 engine carries 406MCA constants. The 449 M18 is measured at
# two frequencies; the engine's def-time default becomes the FIRST (5 Hz)
# drive and the 18 Hz analysis passes its frequency explicitly. The engine's
# gate functions read the offset band as module globals at call time, so
# patching the module keeps every shared gate consistent.
MODEL_NAME = "449M18"
LOW_FREQUENCY_HZ = 5.0
HIGH_FREQUENCY_HZ = 18.0
EXPECTED_FREQUENCY_HZ = LOW_FREQUENCY_HZ
# PLACEHOLDER offset band - TP443's offset page was not available when this
# build was written. The DC offset is always read and recorded; it is only
# gated (immediate high-side FAIL, settled low-side FAIL) while
# OFFSET_GATE_ENABLED is True. Fill in the real TP443 limits, then enable.
OFFSET_GATE_ENABLED = False
OFFSET_MIN_V = 0.3
OFFSET_MAX_V = 3.0
...
# TP443 sensitivity setup for the 449 M18: ONE filter (-25) at both
# frequencies; the legacy limits are scope volts read on the legacy fixture,
# expressed here in mV. FILTER_SPECS_MV is the SAME dict object inside the
# shared engine, this module, the setup-step combobox, and the simulator -
# mutate it IN PLACE so every reader stays synchronized. It keeps the
# engine's historical shape (filter -> legacy minimum at the FIRST frequency);
# the per-frequency minima and the ratio limits live in the TP443_* constants.
TP443_MIN_LEGACY_LOW_MV = 1200.0     # spec 1: >= 1.2 V at 5 Hz
TP443_MIN_LEGACY_HIGH_MV = 720.0     # spec 2: >= 0.72 V at 18 Hz
TP443_RATIO_MIN = 0.70               # spec 3: 18 Hz / 5 Hz ratio band
TP443_RATIO_MAX = 1.30
TP443_TRAY_RATIO_THRESHOLD = 0.72    # spec 4: ratio <= this -> measure tray 100 %
FILTER_SPECS_MV.clear()
FILTER_SPECS_MV.update({"-25 filter": TP443_MIN_LEGACY_LOW_MV})
FILTER_MINIMA_MV = {
    "-25 filter": (TP443_MIN_LEGACY_LOW_MV, TP443_MIN_LEGACY_HIGH_MV),
}


def simulate_offset_v(case_name: str) -> float:  # noqa: F811 - 449 M18 override
    """Simulator offsets inside the placeholder 0.3-3.0 V band."""
    ...
    return _random.uniform(0.8, 1.6)
...
from esp32_backend import (
    ...
    REFERENCE_PWM_DUTY_CYCLE_PERCENT,
    Esp32BackendError,
    Esp32EmitterRig,
    StreamSample,
...
# TP443 uses the -25 filter at both frequencies; the combobox keeps the
# per-batch choice so a second setup can be added later without a UI change.
DEFAULT_FILTER_SETUP = "-25 filter"
...
FAILURE_MODE_CHOICES = (
    ...
    "O - No sensitivity",
    "LS - Low sensitivity",
    "FT - Frequency tracking (18/5 Hz ratio)",
    "N - Noisy",
    "FN - Fast noise",
    "OSC - Oscillation",
    "HO - High offset",
...
FREQUENCY_TRACKING_FAILURE_MODE = "FT - Frequency tracking (18/5 Hz ratio)"
...
# 449 M18 / TP443: the DUT is driven at 5 Hz and then 18 Hz, both with the
# legacy fixture's 20/80 blade duty (20 % ON); the backend programs the board
# with PWM,FREQ + PWM,DUTY on every activation (firmware v3.2 required). The
# permanently mounted AIN1 reference unit is a 406MCA sensor, so every
# reference phase (calibration and the per-test gate) drives the emitter at
# that model's qualified 10 Hz / 50 % instead.
EMITTER_PWM_CHANNEL = "GPIO33"
EMITTER_PWM_FREQUENCY_HZ = LOW_FREQUENCY_HZ          # first TP443 drive
EMITTER_PWM_HIGH_FREQUENCY_HZ = HIGH_FREQUENCY_HZ    # second TP443 drive
REFERENCE_PWM_FREQUENCY_HZ = 10.0
EMITTER_PWM_DUTY_CYCLE = PWM_DUTY_CYCLE_PERCENT      # 20 % ON (20/80 blade)
REFERENCE_PWM_DUTY_CYCLE = REFERENCE_PWM_DUTY_CYCLE_PERCENT
# TP443 counter tolerances: 5 Hz +/- 0.05 Hz, 18 Hz +/- 0.18 Hz. Applied to the
# mean cadence of the sync validation cycles (single cycles get the one-sample
# quantization allowance on top - see stability_analysis).
SYNC_FREQUENCY_TOLERANCE_HZ = {
    LOW_FREQUENCY_HZ: 0.05,
    HIGH_FREQUENCY_HZ: 0.18,
}
...
# Reference gate is ENABLED (2026-09-09). It was off from 2026-08-17 because
# the old buffer/voltage follower was a dual op-amp with no channel
# isolation: the sensor under test coupled into the AIN1 reference signal
# (a SHORTED DUT dragged the reference reading down ~90 %, 4.94 -> 0.30 mV,
# and the app demanded a recalibration for a fault that was the part's),
# which made every baseline worthless the moment another part was loaded.
# The replacement buffer (TI OPA2196 dual precision op-amp, installed
# 2026-09) is channel-isolated, so the emitter-health check runs again
# before every sensor.
#
...
#
# This gate is INDEPENDENT of the model's own CALIBRATION PENDING state: it
# judges the fixture's emitter, not the 449 part, so it is live here while
# SENSITIVITY_GATE_ENABLED is still False.
#
# Every stored baseline predates the new buffer board, so
# REFERENCE_CALIBRATION_SCHEMA_VERSION was bumped to 5 in the same change:
# those files are rejected on load and the app locks testing until
# "Calibrate reference unit" is run fresh on the isolated hardware. Do not
# copy an old baseline forward.
...
# A connected part presents its DC offset on AIN0 through the buffer; a
...
# rail: a bad high-offset part rails AIN0 at the ADC full scale (~5 V), so a
# high reading is a real part (an immediate HO failure once the offset band
# is gated), never a wiring error.
SENSOR_OFFSET_MIN_PLAUSIBLE_V = 0.05
# A JUST-inserted part can also read below the plausibility floor (or well
# below its settled offset) for several seconds while the pyroelectric
# element's DC level wakes up (seen on the 405 M22 lot-500 comparison).
# Poll this long before declaring the slot empty.
...
# Paired-fixture sensitivity calibration - PENDING for the 449 M18.
#
# The legacy TP443 fixture chops a continuously hot 500 K blackbody with a
# mechanical 20/80 blade, so the source modulation is the same at 5 Hz and
# 18 Hz and the scope reading isolates the detector's frequency response.
# This rig pulses the IR emitter electrically instead: the emitter's own
# thermal swing per pulse is smaller at 18 Hz than at 5 Hz, so the raw 18 Hz
# reading is depressed relative to the legacy fixture by a DIFFERENT amount
# than the 5 Hz reading. The fix is one calibration factor PER FREQUENCY,
# derived exactly like the 405 M22's lot-500 factor (the same 30-50 parts
# measured on both fixtures, all at 5 Hz first and then all at 18 Hz per
# TP443 note 2; per-part legacy/raw ratio -> median + regression-through-
# origin agreement -> K_f), validated on withheld parts for the corrected
# 5 Hz value, the corrected 18 Hz value, the corrected ratio, AND identical
# pass/fail decisions around 1.2 V, 0.72 V, 0.70 and 1.30:
#
#     legacy_equivalent_5  = K_5  * raw_5
#     legacy_equivalent_18 = K_18 * raw_18
#     corrected ratio      = (K_18 * raw_18) / (K_5 * raw_5)
#
# Until that comparison exists the factors are 1.0 placeholders and
# SENSITIVITY_GATE_ENABLED is False: the app still computes and records the
# legacy-equivalent values and the ratio (raw AND corrected columns), but it
# applies NONE of the TP443 spec 1-3 limits and stamps every verdict
# CALIBRATION PENDING. Flip the flag, fill in the factors and bump the
# calibration id in the same change, then re-run the withheld parts.
SENSITIVITY_GATE_ENABLED = False
SENSITIVITY_LEGACY_EQUIVALENT_FACTORS: dict[float, float] = {
    LOW_FREQUENCY_HZ: 1.0,
    HIGH_FREQUENCY_HZ: 1.0,
}
SENSITIVITY_CALIBRATION_ID = "449m18_tp443_uncalibrated_v0"
CALIBRATION_PENDING_WARNING_PREFIX = "Calibration pending:"
CALIBRATION_PENDING = "CALIBRATION PENDING"
# TP443 says nothing about polarity; every Eltec model tested on this rig so
# far peaks while the emitter is on, so the same POSITIVE gate is applied at
# both frequencies. Set this False (or change EXPECTED_POLARITY) if the 449
# M18 turns out to be specified the other way.
POLARITY_GATE_ENABLED = True
EXPECTED_POLARITY = POSITIVE_POLARITY
# Capture policy per frequency: same v6.1 structure (5 consecutive peak
# deltas at or below the tracked threshold, then a measurement window that
# must stay within it, 3 attempts). 5 Hz cycles last 200 ms and 18 Hz cycles
# 56 ms, so the windows are 4 s and 2 s; the 30 s deadline covers the
# thermal settle of the emitter/part after PWM-on.
STABILITY_TIMEOUT_S = 30.0
DUT_STABILITY_CONFIRMATION_DELTAS = 5
SENSITIVITY_MEASUREMENT_CYCLES: dict[float, int] = {
    LOW_FREQUENCY_HZ: 20,
    HIGH_FREQUENCY_HZ: 36,
}
# Stability is judged per BLOCK of drive cycles (see
# stability_analysis.block_sync_cycles). At 5 Hz a cycle is 200 samples and
# the robust peak (median of the top 10 %) is phase-tolerant, so one cycle
# per block, like the 406MCA's 10 Hz. At 18 Hz a cycle is only 55.56 samples
# and consecutive cycles sample the pulse at different phases; 9 cycles =
# exactly 500 samples, after which the phase pattern repeats, so the
# per-block robust peak is repeatable. The 36-cycle 18 Hz window is 4 blocks;
# the 5-delta confirmation is 5 blocks (2.5 s). CSV telemetry for the 18 Hz
# capture (stab18_capture_cycles, stab18_measurement_cycles, stabilization
# cycle) therefore counts BLOCKS of 9 cycles.
STABILITY_CYCLES_PER_BLOCK: dict[float, int] = {
    LOW_FREQUENCY_HZ: 1,
    HIGH_FREQUENCY_HZ: 9,
}
MAX_MEASUREMENT_ATTEMPTS = DEFAULT_MAX_MEASUREMENT_ATTEMPTS
# Five validation cycles: 1 s at 5 Hz, 0.28 s at 18 Hz. At 18 Hz the mean over
# five whole-sample periods is quantized to ~0.065 Hz, inside TP443's 0.18.
SYNC_VALIDATION_CYCLES = 5
# Live preview window: 3000 samples = 3 s = 15 cycles at 5 Hz, 54 at 18 Hz.
STREAM_PREVIEW_MAX_SAMPLES = 3000
# Simulator length per frequency: 45 s covers stabilization (~9-13 s), the
# measurement window, and the 30 s "Never stabilizes" timeout.
SIM_CAPTURE_SECONDS = 45.0
...
# How a STALLED stream is attributed (2026-09-03, ported from the 405 M22).
# "Stalled" means the host saw no sample for STREAM_TIMEOUT_S; that alone
# cannot say which side went quiet, so the capture stops the stream first
# and reads the board's STREAM,END reply, whose numbers decide the tag
# below. The tag is appended to the error text and therefore lands in the
# batch's *_attempts.csv (see the 449 README, serial-stream reliability):
...
# Schema v4 marked baselines whose reference readings were driven at the
# 406MCA reference unit's qualified 10 Hz / 50 % on the gain-1 front end -
# the same drive this build uses for its reference phases, so a baseline
# recorded by the 405 M22 build is compatible (both models moved to v5
# together, which keeps that interchange). Schema v5 (2026-09-09) marks
# baselines recorded through the CHANNEL-ISOLATED buffer board: every v4
# file was measured on the shared dual op-amp, where a loaded DUT dragged
# the reference down to a fraction of its true value, so those means and
# windows describe the crosstalk, not the emitter. Older schemas are
# rejected, which forces a fresh "Calibrate reference unit" run.
REFERENCE_CALIBRATION_SCHEMA_VERSION = 5
# TP443-specific simulator cases on top of the shared engine's list.
SIM_TRACKING_CASES = (
    "Borderline sensitivity",
    "Never stabilizes",
    "Low 18 Hz sensitivity",
    "Low 18 Hz ratio",
    "High 18 Hz ratio",
)
for _sim_case in SIM_TRACKING_CASES:
    if _sim_case not in SIM_CASES:
        SIM_CASES = [*SIM_CASES, _sim_case]


def sensitivity_factor(frequency_hz: float) -> float:
    """The fixture calibration factor K_f for one TP443 drive frequency."""
    for known_hz, factor in SENSITIVITY_LEGACY_EQUIVALENT_FACTORS.items():
        if math.isclose(float(frequency_hz), known_hz, abs_tol=1e-6):
            return float(factor)
    raise ValueError(f"No sensitivity calibration factor for {frequency_hz:g} Hz")


def legacy_equivalent_sensitivity_mv(raw_sensitivity_mv: float, frequency_hz: float) -> float:
    """Translate a raw ESP32 pk-pk reading at one frequency to the legacy scale."""
    return float(raw_sensitivity_mv) * sensitivity_factor(frequency_hz)


def tp443_minimum_legacy_mv(filter_setup: str, frequency_hz: float) -> float:
    """TP443 spec 1 (5 Hz) or spec 2 (18 Hz) minimum, legacy scope mV."""
    try:
        low_min_mv, high_min_mv = FILTER_MINIMA_MV[filter_setup]
    except KeyError as exc:
        raise ValueError(f"Unknown filter/setup for sensitivity policy: {filter_setup}") from exc
    if math.isclose(float(frequency_hz), LOW_FREQUENCY_HZ, abs_tol=1e-6):
        return float(low_min_mv)
    if math.isclose(float(frequency_hz), HIGH_FREQUENCY_HZ, abs_tol=1e-6):
        return float(high_min_mv)
    raise ValueError(f"TP443 has no sensitivity limit at {frequency_hz:g} Hz")


def sensitivity_outcome(legacy_mv: float | None, minimum_mv: float) -> str:
    """PASS/FAIL of one frequency's legacy-equivalent value (blank if missing)."""
    if legacy_mv is None or not math.isfinite(legacy_mv):
        return ""
    return OUTCOME_PASS if legacy_mv >= minimum_mv else OUTCOME_FAIL


def ratio_18_over_5(low_mv: float | None, high_mv: float | None) -> float | None:
    """TP443 spec 3 ratio: the 18 Hz reading divided by the 5 Hz reading."""
    if low_mv is None or high_mv is None:
        return None
    if not (math.isfinite(low_mv) and math.isfinite(high_mv)) or low_mv <= 0.0:
        return None
    return float(high_mv) / float(low_mv)


def ratio_outcome(ratio: float | None) -> str:
    """PASS/FAIL of the corrected 18/5 ratio against 0.70-1.30 (blank if missing)."""
    if ratio is None or not math.isfinite(ratio):
        return ""
    return OUTCOME_PASS if TP443_RATIO_MIN <= ratio <= TP443_RATIO_MAX else OUTCOME_FAIL


def simulated_legacy_sensitivity_mv(case_name: str, frequency_hz: float) -> float:
    """Legacy-scale pk-pk the simulator targets for one case and frequency."""
    low_min_mv = TP443_MIN_LEGACY_LOW_MV
    if case_name == "Low sensitivity":
        low_mv = low_min_mv * 0.62
    elif case_name == "Borderline sensitivity":
        # 2 % above the minimum: sampling reads a pk-pk fractionally low, so
        # a target exactly on 1.2 V would fail spec 1 by a few tens of µV.
        low_mv = low_min_mv * 1.02
    elif case_name == "Random good sensor":
        low_mv = low_min_mv * 1.40
    else:
        low_mv = low_min_mv * 1.35
    if math.isclose(float(frequency_hz), LOW_FREQUENCY_HZ, abs_tol=1e-6):
        return low_mv
    # 18 Hz: a healthy part tracks at ~0.95 of its 5 Hz reading; the ratio
    # cases sit clearly outside 0.70-1.30; the low-18 case misses the 0.72 V
    # minimum (and, at 0.48 of 5 Hz, the ratio too); the borderline case
    # sits exactly on the 0.72 tray-sampling threshold - spec 3 passes
    # (>= 0.70) while spec 4 flags the tray.
    if case_name == "Low 18 Hz ratio":
        return low_mv * 0.55
    if case_name == "High 18 Hz ratio":
        return low_mv * 1.45
    if case_name == "Low 18 Hz sensitivity":
        return TP443_MIN_LEGACY_HIGH_MV * 0.80
    if case_name == "Borderline sensitivity":
        return low_mv * TP443_TRAY_RATIO_THRESHOLD
    return low_mv * 0.95
...
def simulate_v6_startup_capture(
    ...
    case_name: str,
    *,
    frequency_hz: float = LOW_FREQUENCY_HZ,
    duration_s: float = SIM_CAPTURE_SECONDS,
    sample_rate_hz: float = DEFAULT_SAMPLE_RATE_HZ,
    duty_fraction: float = EMITTER_PWM_DUTY_CYCLE / 100.0,
    offset_v: float | None = None,
) -> tuple[np.ndarray, np.ndarray, float, float]:
    """Generate a deterministic PWM-on drift followed by a stable waveform.

    One call per frequency. The response to each 20 % pulse rises during the
    ON interval and decays over the OFF interval (a pulse response, so the
    POSITIVE polarity check sees the peak after the rising sync edge), and
    the sync line is high for exactly ``duty_fraction`` of every cycle.
    ``offset_v`` lets the caller reuse one part's offset for both frequencies.
    """
    del filter_setup  # one TP443 setup; kept for signature parity
    ...
    legacy_mv = simulated_legacy_sensitivity_mv(case_name, frequency_hz)
    sensitivity_mv = legacy_mv / sensitivity_factor(frequency_hz)

    duty = min(0.95, max(0.05, float(duty_fraction)))
    rise = phase / duty
    fall = 1.0 - (phase - duty) / (1.0 - duty)
    pulse = np.where(phase < duty, rise, fall) - 0.5   # -0.5 .. +0.5
    ...
    # A 100 mV exponential baseline transient crosses the 0.1 mV adjacent-
    # cycle DUT threshold after ~13 s at 5 Hz and ~9 s at 18 Hz.
    startup_drift_v = 0.100 * np.exp(-t / 3.0)
    if case_name == "Never stabilizes":
        # +/-1.25 mV per stability BLOCK (one cycle at 5 Hz, nine at 18 Hz)
        # keeps every adjacent block-peak delta at 2.5 mV, far above the DUT
        # threshold, so this case never qualifies at either drive. Alternating
        # per cycle would be averaged away by the 18 Hz nine-cycle blocks.
        block_number = np.floor(t * frequency_hz / cycles_per_block_for(frequency_hz)).astype(int)
        startup_drift_v += np.where(block_number % 2 == 0, 0.00125, -0.00125)
    seed = sum((index + 1) * ord(char) for index, char in enumerate(case_name))
    rng = np.random.default_rng(seed + int(round(frequency_hz)))
    noise_v = rng.normal(0.0, 0.00001, sample_count)
    waveform_v = (
        offset_v
        + startup_drift_v
        + pulse * (sensitivity_mv / 1000.0)
        + noise_v
    )
    sync_v = np.where(phase < duty, 1.0, 0.0)
    return waveform_v, sync_v, sample_rate_hz, float(offset_v)
...
def analyze_esp32_waveform(*args, **kwargs) -> WaveformMetrics:
    ...
    # first TP443 drive for callers that do not pass a frequency explicitly.
...
@dataclass
class FrequencyTrackingReport:
    """Everything TP443 asks for, per part: both readings, the ratio, the gates.

    Raw values are the ESP32 pk-pk readings in mV; ``legacy_*`` values apply
    the per-frequency fixture factors (1.0 placeholders until calibrated).
    ``*_outcome`` fields are PASS/FAIL only while the sensitivity gate is
    enabled; otherwise they stay blank and the report is CALIBRATION PENDING.
    """

    filter_setup: str = DEFAULT_FILTER_SETUP
    low_hz: float = LOW_FREQUENCY_HZ
    high_hz: float = HIGH_FREQUENCY_HZ
    raw_low_mv: float | None = None
    raw_high_mv: float | None = None
    factor_low: float = 1.0
    factor_high: float = 1.0
    legacy_low_mv: float | None = None
    legacy_high_mv: float | None = None
    min_legacy_low_mv: float = TP443_MIN_LEGACY_LOW_MV
    min_legacy_high_mv: float = TP443_MIN_LEGACY_HIGH_MV
    ratio_raw: float | None = None
    ratio_corrected: float | None = None
    ratio_min: float = TP443_RATIO_MIN
    ratio_max: float = TP443_RATIO_MAX
    tray_ratio_threshold: float = TP443_TRAY_RATIO_THRESHOLD
    gate_enabled: bool = False
    calibration_id: str = SENSITIVITY_CALIBRATION_ID
    low_outcome: str = ""
    high_outcome: str = ""
    ratio_outcome: str = ""
    tray_100_percent: bool = False
    polarity_low: str = ""
    polarity_high: str = ""
    snr_low_db: float | None = None
    snr_high_db: float | None = None
    measured_low_hz: float | None = None
    measured_high_hz: float | None = None

    @property
    def calibration_pending(self) -> bool:
        return not self.gate_enabled

    @classmethod
    def from_metrics(
        cls,
        metrics_low: WaveformMetrics | None,
        metrics_high: WaveformMetrics | None,
        *,
        filter_setup: str,
        gate_enabled: bool | None = None,
    ) -> "FrequencyTrackingReport":
        gate = SENSITIVITY_GATE_ENABLED if gate_enabled is None else bool(gate_enabled)

        def usable(metrics: WaveformMetrics | None) -> float | None:
            if metrics is None or not metrics.stabilized:
                return None
            value = float(metrics.sensitivity_mv)
            return value if math.isfinite(value) else None

        raw_low = usable(metrics_low)
        raw_high = usable(metrics_high)
        factor_low = sensitivity_factor(LOW_FREQUENCY_HZ)
        factor_high = sensitivity_factor(HIGH_FREQUENCY_HZ)
        legacy_low = None if raw_low is None else raw_low * factor_low
        legacy_high = None if raw_high is None else raw_high * factor_high
        min_low = tp443_minimum_legacy_mv(filter_setup, LOW_FREQUENCY_HZ)
        min_high = tp443_minimum_legacy_mv(filter_setup, HIGH_FREQUENCY_HZ)
        ratio_raw = ratio_18_over_5(raw_low, raw_high)
        ratio_corrected = ratio_18_over_5(legacy_low, legacy_high)
        report = cls(
            filter_setup=filter_setup,
            raw_low_mv=raw_low,
            raw_high_mv=raw_high,
            factor_low=factor_low,
            factor_high=factor_high,
            legacy_low_mv=legacy_low,
            legacy_high_mv=legacy_high,
            min_legacy_low_mv=min_low,
            min_legacy_high_mv=min_high,
            ratio_raw=ratio_raw,
            ratio_corrected=ratio_corrected,
            gate_enabled=gate,
            polarity_low="" if metrics_low is None else metrics_low.polarity,
            polarity_high="" if metrics_high is None else metrics_high.polarity,
            snr_low_db=None if metrics_low is None else metrics_low.signal_to_noise_db,
            snr_high_db=None if metrics_high is None else metrics_high.signal_to_noise_db,
            measured_low_hz=None if metrics_low is None else metrics_low.measured_frequency_hz,
            measured_high_hz=None if metrics_high is None else metrics_high.measured_frequency_hz,
        )
        if gate:
            report.low_outcome = sensitivity_outcome(legacy_low, min_low)
            report.high_outcome = sensitivity_outcome(legacy_high, min_high)
            report.ratio_outcome = ratio_outcome(ratio_corrected)
            # TP443 spec 4: one sample part failing any spec, or a ratio of
            # 0.72 or less (even though the part itself passes spec 3 down
            # to 0.70), means the whole tray must be measured.
            report.tray_100_percent = (
                OUTCOME_FAIL in (report.low_outcome, report.high_outcome, report.ratio_outcome)
                or (
                    ratio_corrected is not None
                    and math.isfinite(ratio_corrected)
                    and ratio_corrected <= TP443_TRAY_RATIO_THRESHOLD
                )
            )
        return report
    ...
    def csv_fields(self) -> dict[str, str]:
        return {
            "low_hz": f"{self.low_hz:g}",
            "high_hz": f"{self.high_hz:g}",
            "sensitivity_5hz_raw_mv": _fmt_optional_float(self.raw_low_mv, 6),
            "sensitivity_5hz_legacy_equivalent_mv": _fmt_optional_float(self.legacy_low_mv, 6),
            "sensitivity_5hz_factor": f"{self.factor_low:.6f}",
            "sensitivity_5hz_min_legacy_mv": f"{self.min_legacy_low_mv:.3f}",
            "sensitivity_5hz_outcome": self.low_outcome,
            "sensitivity_18hz_raw_mv": _fmt_optional_float(self.raw_high_mv, 6),
            "sensitivity_18hz_legacy_equivalent_mv": _fmt_optional_float(self.legacy_high_mv, 6),
            "sensitivity_18hz_factor": f"{self.factor_high:.6f}",
            "sensitivity_18hz_min_legacy_mv": f"{self.min_legacy_high_mv:.3f}",
            "sensitivity_18hz_outcome": self.high_outcome,
            "ratio_18_over_5_raw": _fmt_optional_float(self.ratio_raw, 6),
            "ratio_18_over_5_corrected": _fmt_optional_float(self.ratio_corrected, 6),
            "ratio_min": f"{self.ratio_min:.3f}",
            "ratio_max": f"{self.ratio_max:.3f}",
            "ratio_outcome": self.ratio_outcome,
            "ratio_tray_100_percent_flag": "YES" if self.tray_100_percent else "NO",
            "sensitivity_calibration_id": self.calibration_id,
            "sensitivity_gate_enabled": "YES" if self.gate_enabled else "NO",
            "polarity_5hz": self.polarity_low,
            "polarity_18hz": self.polarity_high,
            "snr_5hz_db": _fmt_optional_float(self.snr_low_db, 2),
            "snr_18hz_db": _fmt_optional_float(self.snr_high_db, 2),
            "measured_5hz_sync_hz": _fmt_optional_float(self.measured_low_hz, 4),
            "measured_18hz_sync_hz": _fmt_optional_float(self.measured_high_hz, 4),
        }


def _frequency_signal_checks(
    frequency_hz: float,
    metrics: WaveformMetrics | None,
    fail_reasons: list[str],
    warnings: list[str],
) -> None:
    """The "is there a real, stable, positive response" checks for one drive.

    These do not depend on the fixture calibration, so they always apply:
    stabilization, polarity, and the SNR gate (a dead/undriven emitter still
    produces noise that can look big enough).
    """
    label = frequency_label(frequency_hz)
    if metrics is None:
        fail_reasons.append(f"{label}: waveform was not measured.")
        return
    unstable_reasons = [
        warning for warning in metrics.warnings if str(warning).startswith("Unstable")
    ]
    warnings.extend(
        f"{label}: {warning}"
        for warning in metrics.warnings
        if not str(warning).startswith("Unstable")
    )
    if not metrics.stabilized:
        # build_stability_timeout_result puts the detailed "Unstable at
        # X Hz: ..." reason in the warnings; reuse it as the fail reason.
        # Polarity/SNR are meaningless without a stable window, so stop here.
        fail_reasons.append(
            unstable_reasons[0]
            if unstable_reasons
            else f"Unstable at {label}: waveform did not stabilize before the capture limit."
        )
        return
    if POLARITY_GATE_ENABLED and metrics.polarity != EXPECTED_POLARITY:
        detail = format_polarity_detail(metrics)
        detail_suffix = "" if detail is None else f" ({detail})"
        fail_reasons.append(
            f"{label}: polarity is {metrics.polarity}; expected {EXPECTED_POLARITY}.{detail_suffix}"
        )
    snr = metrics.signal_to_noise_ratio
    if snr is None:
        fail_reasons.append(
            f"{label}: signal quality could not be verified (SNR unavailable) - "
            "confirm the emitter is powered and driving before retesting."
        )
        ...
        fail_reasons.append(
            f"{label}: signal-to-noise too low: SNR {snr:.2f}{db_text} (minimum "
            f"{MIN_SIGNAL_TO_NOISE_RATIO:.1f}). This looks like sensor noise with no "
            "emitter response - check the emitter drive."
        )


def evaluate_tracking_result(
    offset_v: float | None,
    metrics_low: WaveformMetrics | None,
    metrics_high: WaveformMetrics | None,
    filter_setup: str,
) -> tuple[FinalResult, FrequencyTrackingReport]:
    """Apply every TP443 gate to one part's 5 Hz + 18 Hz captures.

    Returns the production ``FinalResult`` (its single ``sensitivity_mv`` /
    ``polarity`` / ``waveform_metrics`` carry the 5 Hz capture, for the
    shared attempt log and batch summary) together with the full
    two-frequency report.
    """
    fail_reasons: list[str] = []
    warnings: list[str] = []
    ...
    elif OFFSET_GATE_ENABLED and (offset_v < OFFSET_MIN_V or offset_v > OFFSET_MAX_V):
        fail_reasons.append(
            f"Offset out of range: {offset_v:.3f} V, expected {OFFSET_MIN_V:.1f} to {OFFSET_MAX_V:.1f} V."
        )

    _frequency_signal_checks(LOW_FREQUENCY_HZ, metrics_low, fail_reasons, warnings)
    _frequency_signal_checks(HIGH_FREQUENCY_HZ, metrics_high, fail_reasons, warnings)

    report = FrequencyTrackingReport.from_metrics(
        metrics_low, metrics_high, filter_setup=filter_setup
    )
    if report.gate_enabled:
        if report.low_outcome == OUTCOME_FAIL:
            fail_reasons.append(
                f"Sensitivity too low at {frequency_label(LOW_FREQUENCY_HZ)}: legacy-equivalent "
                f"{report.legacy_low_mv:.1f} mV is below the TP443 minimum "
                f"{report.min_legacy_low_mv:.0f} mV (raw {report.raw_low_mv:.3f} mV x "
                f"{report.factor_low:.3f})."
            )
        if report.high_outcome == OUTCOME_FAIL:
            fail_reasons.append(
                f"Sensitivity too low at {frequency_label(HIGH_FREQUENCY_HZ)}: legacy-equivalent "
                f"{report.legacy_high_mv:.1f} mV is below the TP443 minimum "
                f"{report.min_legacy_high_mv:.0f} mV (raw {report.raw_high_mv:.3f} mV x "
                f"{report.factor_high:.3f})."
            )
        if report.ratio_outcome == OUTCOME_FAIL:
            fail_reasons.append(
                f"Frequency tracking ratio out of range: 18 Hz / 5 Hz = "
                f"{report.ratio_corrected:.3f}, expected {report.ratio_min:.2f} to "
                f"{report.ratio_max:.2f} (raw ratio {report.ratio_raw:.3f})."
            )
        elif (
            report.ratio_corrected is not None
            and math.isfinite(report.ratio_corrected)
            and report.ratio_corrected <= report.tray_ratio_threshold
        ):
            warnings.append(
                f"Ratio {report.ratio_corrected:.3f} is at or below "
                f"{report.tray_ratio_threshold:.2f}: the part passes spec 3, but TP443 "
                "spec 4 requires the whole tray to be measured 100 %."
            )
    else:
        warnings.append(
            f"{CALIBRATION_PENDING_WARNING_PREFIX} the TP443 limits (1.2 V at 5 Hz, "
            "0.72 V at 18 Hz, ratio 0.70-1.30) are NOT applied - the per-frequency "
            "fixture factors have not been derived yet. Raw readings and the raw "
            "ratio are recorded; this verdict only says the part gave a stable, "
            f"{EXPECTED_POLARITY.lower()} response at both frequencies."
        )

    final = FinalResult(
        passed=not fail_reasons,
        offset_v=offset_v,
        # The raw 5 Hz reading, None when that capture never stabilized (a
        # timeout's diagnostic metrics carry a placeholder 0.0).
        sensitivity_mv=report.raw_low_mv,
        polarity="" if metrics_low is None else metrics_low.polarity,
        fail_reasons=fail_reasons,
        warnings=warnings,
        waveform_metrics=metrics_low,
    )
    return final, report


def is_calibration_pending(final_result: FinalResult | None) -> bool:
    """True when a verdict was produced without the TP443 limits applied."""
    if final_result is None:
        return False
    return any(
        str(warning).startswith(CALIBRATION_PENDING_WARNING_PREFIX)
...
# Per-capture stability telemetry (one set per TP443 drive, prefixed
# ``stab5_`` and ``stab18_`` in the batch CSV).
STABILITY_TELEMETRY_FIELDS = (
    "stabilized",
    "stability_timeout",
...
STABILITY_LOW_PREFIX = "stab5_"
STABILITY_HIGH_PREFIX = "stab18_"


def stability_fields(prefix: str) -> list[str]:
    return [f"{prefix}{name}" for name in STABILITY_TELEMETRY_FIELDS]
...
CSV_FIELDS = [
    ...
    # taken after both captures; comparing the two columns shows each part's
    # settling behavior.
    "offset_initial_v",
    # ``sensitivity_mv`` is the RAW 5 Hz reading, kept under the shared name
    # for the attempt log and the batch summary tooling. The explicit
    # per-frequency columns below carry the raw, corrected and gated values.
    "sensitivity_mv",
    "sensitivity_5hz_raw_mv",
    "sensitivity_5hz_legacy_equivalent_mv",
    "sensitivity_5hz_factor",
    "sensitivity_5hz_min_legacy_mv",
    "sensitivity_5hz_outcome",
    "sensitivity_18hz_raw_mv",
    "sensitivity_18hz_legacy_equivalent_mv",
    "sensitivity_18hz_factor",
    "sensitivity_18hz_min_legacy_mv",
    "sensitivity_18hz_outcome",
    # TP443 spec 3 (ratio) and spec 4 (tray sampling flag).
    "ratio_18_over_5_raw",
    "ratio_18_over_5_corrected",
    "ratio_min",
    "ratio_max",
    "ratio_outcome",
    "ratio_tray_100_percent_flag",
    "sensitivity_calibration_id",
    "sensitivity_gate_enabled",
    # ``polarity`` = the 5 Hz polarity (shared name); both are also explicit.
    "polarity",
    "polarity_good_bad",
    "polarity_5hz",
    "polarity_18hz",
    ...
    "snr_5hz_db",
    "snr_18hz_db",
    "measured_5hz_sync_hz",
    "measured_18hz_sync_hz",
    ...
    "reference_check_mv",
    "reference_drift_pct",
    # V6 peak-delta stabilization telemetry, once per drive.
    *stability_fields(STABILITY_LOW_PREFIX),
    *stability_fields(STABILITY_HIGH_PREFIX),
...
def results_root_dir() -> Path:
    ...
    return Path.home() / "Documents" / "Eltec_449M18_Test_Results" / "449m18_esp32"
...
def batch_results_path(batch_number: str) -> Path:
    return results_root_dir() / f"449m18_esp32_lot_{safe_filename_part(batch_number)}.csv"
...
def reference_calibration_path() -> Path:
    """Persistent AIN1 emitter/reference baseline for this 449 M18 build.

    The reference unit is driven at its own 10 Hz / 50 % in every build, but
    each model keeps its baseline in its own results folder so a fixture
    change for one model never silently re-gates another.
    """
...
def load_reference_calibration(path: Path | None = None) -> ReferenceCalibration | None:
    # No cross-model fallback: each build keeps its own baseline file so a
    # fixture change for one model never silently re-gates another.
...
def next_sensor_number_for_batch(csv_path: Path) -> int:
    ...
    so it is only spent when a part actually passes. A FAIL - or a NOT
    MEASURED row - leaves the number open and the next part loaded into the
    rig is tested under it again, which is what the bench already does
    physically: a bad part is put aside and another one takes its place.
    A batch CSV is therefore one row per TEST, not one row per number:
    500-7 can appear several times, the last of them the PASS that finally
    earns the number.
...
def suggest_failure_mode(final_result: FinalResult | None) -> str:
    ...
    # Offset is measured before the captures, so an electrical offset failure
    # remains primary even when the subsequent waveforms are unstable, weak,
    # reversed, or noisy. Only while the offset band is actually gated - with
    # the placeholder band an out-of-band offset is not a failure.
    offset_v = final_result.offset_v
    if "no offset:" in reason_text:
        return "D - No offset"
    if OFFSET_GATE_ENABLED and offset_v is not None and math.isfinite(offset_v):
    ...
    # TP443 spec 3: the 18/5 Hz ratio is its own failure type.
    if "ratio out of range" in reason_text:
        return FREQUENCY_TRACKING_FAILURE_MODE
    if "polarity" in reason_text:
        return "RP - Reversed polarity"
    if no_coherent_response:
        if (
            offset_v is not None
            and math.isfinite(offset_v)
            and OFFSET_MIN_V <= offset_v <= OFFSET_MAX_V
        ):
            return "GO/D - Good offset/no signal"
...
def append_result_csv(
    ...
    *,
    ...
    tracking_report: "FrequencyTrackingReport | None" = None,
    capture_report: "StabilityCaptureReport | None" = None,
    capture_report_high: "StabilityCaptureReport | None" = None,
    ...
    """Write one verdict row: both TP443 readings, the ratio, and the gates.

    ``capture_report`` is the 5 Hz stability telemetry, ``capture_report_high``
    the 18 Hz one; ``tracking_report`` carries the per-frequency raw and
    legacy-equivalent readings, the ratio, and the gate outcomes. ``pwm_hz``
    is accepted for signature parity with the single-frequency models and
    ignored: this model records both drives in ``low_hz`` / ``high_hz``.
    """
    del pwm_hz
    ...
    row = {
        ...
        "low_hz": f"{LOW_FREQUENCY_HZ:g}",
        "high_hz": f"{HIGH_FREQUENCY_HZ:g}",
        ...
        "sensitivity_mv": _fmt_optional_float(final_result.sensitivity_mv, 6),
        "sensitivity_calibration_id": SENSITIVITY_CALIBRATION_ID,
        "sensitivity_gate_enabled": "YES" if SENSITIVITY_GATE_ENABLED else "NO",
    ...
    if tracking_report is not None:
        row.update(tracking_report.csv_fields())
    if capture_report is not None:
        row.update(capture_report.csv_fields(prefix=STABILITY_LOW_PREFIX))
    if capture_report_high is not None:
        row.update(capture_report_high.csv_fields(prefix=STABILITY_HIGH_PREFIX))
...
def snapshot_detail_lines(
    ...
    *,
    frequency_hz: float = LOW_FREQUENCY_HZ,
    tracking: "FrequencyTrackingReport | None" = None,
) -> list[str]:
    """PNG footer lines for one drive's capture (5 Hz or 18 Hz)."""
    label = frequency_label(frequency_hz)
    sensitivity_text = "Not measured (unstable)"
    raw_sensitivity_text = ""
    if metrics.stabilized:
        sensitivity_text = (
            f"{legacy_equivalent_sensitivity_mv(metrics.sensitivity_mv, frequency_hz):.1f} mV "
            f"legacy-equivalent (x{sensitivity_factor(frequency_hz):.3f}"
            + (", CALIBRATION PENDING)" if not SENSITIVITY_GATE_ENABLED else ")")
        )
    ...
    lines = [
        ...
        f"Sensor: {sensor_id}",
        f"Drive: {label} / {EMITTER_PWM_DUTY_CYCLE:g}% duty (TP443 20/80 blade equivalent)",
        f"Sensitivity {label}: {sensitivity_text}",
        f"Polarity {label}: {polarity_text}",
    ]
    if raw_sensitivity_text:
        lines.insert(4, f"Raw sensitivity {label}: {raw_sensitivity_text}")
    ...
    if tracking is not None and tracking.ratio_raw is not None:
        ratio_text = f"Ratio 18/5 Hz: raw {tracking.ratio_raw:.3f}"
        if tracking.ratio_corrected is not None:
            ratio_text += f", corrected {tracking.ratio_corrected:.3f}"
        if tracking.ratio_outcome:
            ratio_text += f" -> {tracking.ratio_outcome}"
...
def measurement_cycles_for(frequency_hz: float) -> int:
    """TP443 measurement window for one drive (20 cycles at 5 Hz, 36 at 18 Hz)."""
    for known_hz, cycles in SENSITIVITY_MEASUREMENT_CYCLES.items():
        if math.isclose(float(frequency_hz), known_hz, abs_tol=1e-6):
            return int(cycles)
    return DEFAULT_MEASUREMENT_CYCLES_REQUIRED


def sync_tolerance_for(frequency_hz: float) -> float:
    """TP443 counter tolerance for one drive; +/-0.1 Hz for anything else."""
    for known_hz, tolerance in SYNC_FREQUENCY_TOLERANCE_HZ.items():
        if math.isclose(float(frequency_hz), known_hz, abs_tol=1e-6):
            return float(tolerance)
    return 0.1


def cycles_per_block_for(frequency_hz: float) -> int:
    """Drive cycles per stability block (1 at 5 Hz, 9 at 18 Hz, else 1)."""
    for known_hz, cycles in STABILITY_CYCLES_PER_BLOCK.items():
        if math.isclose(float(frequency_hz), known_hz, abs_tol=1e-6):
            return int(cycles)
    return 1


def measurement_blocks_for(frequency_hz: float) -> int:
    """The measurement window in stability blocks for one drive."""
    cycles = measurement_cycles_for(frequency_hz)
    per_block = cycles_per_block_for(frequency_hz)
    if cycles % per_block:
        raise ValueError(
            f"{cycles} measurement cycles at {frequency_hz:g} Hz is not a whole "
            f"number of {per_block}-cycle blocks"
        )
    return cycles // per_block


def stability_sync_for(sync_v, frequency_hz: float) -> np.ndarray:
    """The sync the peak-delta rule analyzes for one drive (blocked at 18 Hz)."""
    return np.asarray(
        block_sync_cycles(sync_v, cycles_per_block_for(frequency_hz)), dtype=float
    )
...
class EmitterEsp32Rig(Esp32EmitterRig):
        ...
        if not math.isclose(header.sample_rate_hz, sample_rate_hz, rel_tol=0.01):
            try:
                ...
                raise Esp32BackendError(
                    ...
                    f"{EXPECTED_FIRMWARE_PREFIX}3.2 or newer."
                )
    ...
    def read_reference_until_stable(
        ...
        *,
        ...
        the emitter at REFERENCE_PWM_FREQUENCY_HZ (10 Hz / 50 %), not one of
        the DUT's TP443 drives.
        """
    ...
    def read_waveform_until_stable(
        ...
        *,
        ...
        sync_frequency_tolerance_hz: float | None = None,
        sync_validation_cycles: int = SYNC_VALIDATION_CYCLES,
        progress=None,
        preview=None,
        cancelled=None,
        channel: str = "sensor",
        ...
        ``measurement_cycles`` defaults to this drive's TP443 window
        (SENSITIVITY_MEASUREMENT_CYCLES) and ``sync_frequency_tolerance_hz``
        to the drive's TP443 counter tolerance (SYNC_FREQUENCY_TOLERANCE_HZ);
        the 10 Hz reference phases fall back to +/-0.1 Hz.
        """
        del waveform_range_v
        if measurement_cycles is None:
            measurement_cycles = measurement_cycles_for(expected_frequency_hz)
        if sync_frequency_tolerance_hz is None:
            sync_frequency_tolerance_hz = sync_tolerance_for(expected_frequency_hz)
        # The peak-delta rule runs on blocks of drive cycles (1 at 5 Hz and
        # for the reference, 9 at 18 Hz); the measurement window is expressed
        # in blocks for it. Sync validation always uses the REAL sync.
        cycles_per_block = cycles_per_block_for(expected_frequency_hz)
        if measurement_cycles % cycles_per_block:
            raise ValueError(
                f"{measurement_cycles} measurement cycles is not a whole number of "
                f"{cycles_per_block}-cycle blocks"
            )
        measurement_cycles = measurement_cycles // cycles_per_block
        self.connect()
        samples = []
        header = self.start_stream(channel)
        if not math.isclose(header.sample_rate_hz, sample_rate_hz, rel_tol=0.01):
            try:
                ...
                raise Esp32BackendError(
                    f"ESP32 advertised {header.sample_rate_hz:g} samples/s; "
                    f"this tester requires {sample_rate_hz:g}. Re-flash {EXPECTED_FIRMWARE_PREFIX}3.2 or newer."
                )
        ...
        diagnostics = None
        stream_data_source = "esp32_reference" if is_reference else "esp32_449m18"
        ...
        try:
            while len(samples) < target_scans:
                ...
                analysis = analyze_stability(
                    waveform_np,
                    block_sync_cycles(sync_np, cycles_per_block),
                    actual_scan_rate,
                    analysis_settings,
                    pwm_elapsed_offset_s=pwm_elapsed_offset_s,
                    stability_deadline_s=stability_timeout_s,
                    ...
                    if (
                        ...
                        try:
                            validate_rising_sync_cycles(
                                ...
                                actual_scan_rate,
                                expected_frequency_hz=expected_frequency_hz,
                                frequency_tolerance_hz=sync_frequency_tolerance_hz,
...
class StabilityCaptureReport:
    ...
    def csv_fields(self, prefix: str = "") -> dict[str, str]:
        """Telemetry columns, optionally prefixed (``stab5_`` / ``stab18_``)."""
        fields = {
...
def analyze_v6_stable_measurement(
    ...
    *,
    ...
    """Apply production signal math to the selected post-stability window.

    ``analysis`` judged stability per block of drive cycles (see
    STABILITY_CYCLES_PER_BLOCK); its measurement segments are blocks, and the
    shared engine re-finds the REAL cycles inside them from ``sync_v``.
    """
    segments = analysis.measurement_segments
    cycles_per_block = cycles_per_block_for(expected_frequency_hz)
    expected_blocks = analysis.report.measurement_cycles_required
    if len(segments) != expected_blocks:
        raise ValueError(
            f"Stable measurement requires {expected_blocks} complete blocks; "
            f"received {len(segments)}."
        )
    expected_cycles = expected_blocks * cycles_per_block
...
def build_stability_timeout_result(
    ...
    *,
    ...
    reason = f"Unstable at {frequency_label(expected_frequency_hz)}: " + detail
    warnings = [
    ...
    fail_reasons = [reason]
    if OFFSET_GATE_ENABLED and not (OFFSET_MIN_V <= offset_v <= OFFSET_MAX_V):
        fail_reasons.insert(
            0,
            f"Offset out of range: {offset_v:.3f} V, expected {OFFSET_MIN_V:.1f} to {OFFSET_MAX_V:.1f} V.",
        )
...
def build_offset_failure_result(
    ...
    *,
    ...
    """Immediate FAIL for an offset violation - nothing else is measured.

    Only reachable while OFFSET_GATE_ENABLED: the offset read is the first
    per-part step, and a part above the band fails on the spot - neither
    frequency is captured, so there is no waveform to attach.
    """
    reason = (
    ...
    note = (
        "The 5 Hz and 18 Hz captures were not run: the offset gate fails the "
        "part immediately."
    )
    metrics = WaveformMetrics(
        sensitivity_mv=0.0,
...
def build_no_output_sensor_result(
    ...
    *,
    ...
    note = "The 5 Hz and 18 Hz captures were not run: the part presents no output."
    metrics = WaveformMetrics(
        sensitivity_mv=0.0,
        sensitivity_amplified_mv=0.0,
        polarity="NOT MEASURED",
...
class EmitterTesterApp(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            pass
        self.title("Eltec 449 M18 ESP32 Tester (5/18 Hz)")
        ...
        # Current-sensor measurement state. ``last_metrics`` /
        # ``last_capture_report`` carry the 5 Hz capture (the shared names the
        # v2.0 skip/attempt machinery expects); the 18 Hz capture and the
        # two-frequency TP443 report sit next to them.
        ...
        self.last_metrics_high: WaveformMetrics | None = None
        self.last_capture_report_high: StabilityCaptureReport | None = None
        self.last_tracking_report: FrequencyTrackingReport | None = None
        ...
        self.stability_diagnostics_high_saved = False

        self.logo_image: tk.PhotoImage | None = None
        self.wave_canvas: ScopeView | None = None
        self.wave_canvas_high: ScopeView | None = None
    ...
    def _build_variables(self) -> None:
        ...
        # "STEP 2/3 — 5 HZ SENSITIVITY" label above the progress bar.
        self.measure_step_var = tk.StringVar(value="")
        self.live_wave_header_var = tk.StringVar(
            value=(
                "LIVE SIGNAL  ·  ADS AIN0 SENSOR + PWM SYNC OVERLAY (HIGH = EMITTER ON, "
                f"{EMITTER_PWM_DUTY_CYCLE:g}% DUTY)"
            )
        )
    ...
    def _redraw_header(self, _event=None) -> None:
        ...
        self.header.create_text(title_x, S(26), anchor="w", text="449 M18 SENSOR TESTER", fill=HEADER_FG, font=self.fd(21), tags="static")
        title_width = tkfont.Font(font=self.fd(21)).measure("449 M18 SENSOR TESTER")
        chip_x = title_x + title_width + S(14)
        draw_round_rect(self.header, chip_x, S(15), chip_x + S(52), S(37), Sf(8), fill=ELTEC_RED, outline="", tags="static")
        self.header.create_text(chip_x + S(26), S(26), text="TP443", fill="#ffffff", font=self.fm(10, "bold"), tags="static")
        if self.simulator_var.get():
        ...
        self.header.create_text(title_x, S(47), anchor="w", text="PYROELECTRIC SENSOR QC  ·  FREQUENCY TRACKING 5 / 18 HZ  ·  EMITTER RIG", fill=mix_color(HEADER_SUB_FG, ELTEC_BLUE, 0.25), font=self.fm(9, "bold"), tags="static")
    ...
    def _build_advanced_panel(self, parent: tk.Widget) -> tk.Frame:
        ...
        if settings is None:
            ...
            stability_rule_text = (
                f"Each drive (5 Hz, then 18 Hz, both {EMITTER_PWM_DUTY_CYCLE:g}% duty) gets its own "
                f"capture: attempts 1, 2, and 3 each require {DUT_STABILITY_CONFIRMATION_DELTAS} "
                f"consecutive cycle-to-cycle peak deltas at or below "
                f"{settings.peak_delta_threshold_mv:.3f} mV, then "
                f"{measurement_cycles_for(LOW_FREQUENCY_HZ)} (5 Hz) / "
                f"{measurement_cycles_for(HIGH_FREQUENCY_HZ)} (18 Hz) measurement cycles that must "
                f"stay within the same limit. A kick discards that window and restarts the check; a "
                f"third kick fails as unstable. Requalification must finish within "
                f"{STABILITY_TIMEOUT_S:g} seconds of PWM-on. An unstable 5 Hz capture skips the 18 Hz one."
            )
        ...
        tk.Label(
            panel,
            text=(f"ESP32 rig (firmware v{'.'.join(str(p) for p in MINIMUM_FIRMWARE_VERSION[:2])}+): "
                  f"ADS AIN1 = fixed reference/emitter gate, "
                  f"ADS AIN0 = buffered DUT (offset + AC), "
                  f"streamed sync = PWM state, {EMITTER_PWM_CHANNEL} = MOSFET gate. "
                  f"Emitter driven at {EMITTER_PWM_FREQUENCY_HZ:g} Hz and then "
                  f"{EMITTER_PWM_HIGH_FREQUENCY_HZ:g} Hz at {EMITTER_PWM_DUTY_CYCLE:g}% duty for the "
                  f"DUT (TP443 20/80 blade equivalent), and {REFERENCE_PWM_FREQUENCY_HZ:g} Hz / "
                  f"{REFERENCE_PWM_DUTY_CYCLE:g}% for the 406MCA reference phases. "
                  ...
                  f"Verdict: TP443 spec 1 (≥ {TP443_MIN_LEGACY_LOW_MV / 1000:g} V at 5 Hz), spec 2 "
                  f"(≥ {TP443_MIN_LEGACY_HIGH_MV / 1000:g} V at 18 Hz) and spec 3 (ratio "
                  f"{TP443_RATIO_MIN:.2f}-{TP443_RATIO_MAX:.2f}) on legacy-equivalent values "
                  f"(raw × per-frequency factor); "
                  + ("gate ENABLED" if SENSITIVITY_GATE_ENABLED else "gate DISABLED - calibration pending")
                  + f" ({SENSITIVITY_CALIBRATION_ID}). "
    ...
    def update_filter_hint(self) -> None:
        try:
            low_min_mv = tp443_minimum_legacy_mv(self.filter_var.get(), LOW_FREQUENCY_HZ)
            high_min_mv = tp443_minimum_legacy_mv(self.filter_var.get(), HIGH_FREQUENCY_HZ)
        except ValueError:
            self.filter_hint_var.set("SENSITIVITY POLICY UNAVAILABLE FOR THIS FILTER")
            return
        limits = (
            f"TP443: ≥ {low_min_mv / 1000:g} V @ 5 Hz  ·  ≥ {high_min_mv / 1000:g} V @ 18 Hz  ·  "
            f"RATIO 18/5 {TP443_RATIO_MIN:.2f}-{TP443_RATIO_MAX:.2f}  ·  "
            f"≤ {TP443_TRAY_RATIO_THRESHOLD:.2f} → MEASURE TRAY 100%"
        )
        if SENSITIVITY_GATE_ENABLED:
            hint = (
                limits
                + f"  ·  FACTORS ×{sensitivity_factor(LOW_FREQUENCY_HZ):.3f} / "
                f"×{sensitivity_factor(HIGH_FREQUENCY_HZ):.3f} ({SENSITIVITY_CALIBRATION_ID})"
            )
        else:
            hint = (
                limits
                + "  ·  GATE DISABLED UNTIL THE PER-FREQUENCY FIXTURE FACTORS ARE "
                "DERIVED (RAW VALUES + RAW RATIO RECORDED)"
            )
    ...
    def _build_load_sensor_card(self, row: int) -> None:
        ...
        tk.Label(text_col, text="Start runs the 5 Hz and 18 Hz captures straight away; after that, load the next sensor and press Next.", bg=CARD_BG, fg=MUTED_FG, font=self.fb(13), wraplength=S(520), justify="left").pack(anchor="w", pady=(6, 0))
        chips = tk.Frame(text_col, bg=CARD_BG)
        chips.pack(anchor="w", pady=(14, 0))
        for chip_text in (
            f"{EMITTER_PWM_FREQUENCY_HZ:g} Hz → {EMITTER_PWM_HIGH_FREQUENCY_HZ:g} Hz · {EMITTER_PWM_DUTY_CYCLE:g}% DUTY",
            "GAIN ×1 BUFFER",
        ):
            chip = tk.Label(chips, text=chip_text, bg=ELTEC_BLUE_LIGHT, fg=ELTEC_BLUE_DARK, font=self.fm(9, "bold"), padx=10, pady=4)
            chip.pack(side="left", padx=(0, 8))
        if not SENSITIVITY_GATE_ENABLED:
            tk.Label(
                text_col,
                text=(
                    "Calibration pending: the TP443 limits are not applied yet. Each "
                    "verdict records the raw 5 Hz / 18 Hz readings and their ratio for "
                    "the paired comparison against the legacy fixture."
                ),
                bg=CARD_BG,
                fg=WARN_FG,
                font=self.fb(10),
    ...
    def render_measure_fault_view(self) -> None:
        ...
        Nothing was written, so the number is untouched: Next simply reads
        this sensor again. Recording it as NOT MEASURED is the way to close
        the number out when the rig itself cannot be fixed on the spot - it
        writes a row with no readings, which is not a pass, so the number
        stays open for the next part anyway.
        """
        self._step_heading(
            0,
            "02",
            f"{self.current_sensor_id}: nothing was recorded",
            "Press Next to read it again, or record this sensor as NOT MEASURED.",
        )
        card = Card(
            self.step_frame,
            card_bg=FAIL_BG,
        ...
        tk.Label(
            ...
            text=(
                ...
                "or polarity and leaves the sensor out of the yield. It is not a pass, so "
                f"the next part is still tested as {self.current_sensor_id}."
            ),
            bg=PAGE_BG,
            fg=MUTED_FG,
            font=self.fb(11),
    ...
    def _build_progress_bar(self, row: int) -> None:
        ...
        The label reads e.g. "STEP 2/3 — 5 HZ SENSITIVITY"; the bar fills
    ...
    def render_result_view(self) -> None:
        ...
        tracking = self.last_tracking_report
        outcome = result_outcome(result)
        passed = outcome == OUTCOME_PASS
        pending = passed and is_calibration_pending(result)
        self._build_result_banner(row=0, outcome=outcome, pending=pending)
        next_row = 1
        if pending:
            # The part gave a stable, positive response at both frequencies,
            # but the TP443 limits are not applied yet - say so on every
            # verdict so a PASS is never mistaken for a qualified one.
            warn_card = Card(
                self.step_frame,
                card_bg=WARN_BG,
            ...
            tk.Label(
                warn_inner,
                text="PASSED - TP443 LIMITS NOT APPLIED (CALIBRATION PENDING)",
                bg=WARN_BG,
                fg=WARN_FG,
                font=self.fm(10, "bold"),
            ...
            tk.Label(
                warn_inner,
                text=self._calibration_pending_message(),
                bg=WARN_BG,
                fg=WARN_FG,
                font=self.fb(11),
                justify="left",
            ...
            next_row += 1
        if tracking is not None and tracking.tray_100_percent:
            # TP443 spec 4: the sampled part triggers a 100 % tray measurement.
            tray_card = Card(
                self.step_frame,
                card_bg=WARN_BG,
                border=mix_color(WARN_ACCENT, WARN_BG, 0.45),
            ...
            tray_card.grid(row=next_row, column=0, sticky="ew", pady=(14, 0))
            tray_inner = tray_card.inner
            tray_inner.columnconfigure(0, weight=1)
            tk.Label(
                tray_inner,
                text="TP443 SPEC 4 - MEASURE THE WHOLE TRAY (100 %)",
                bg=WARN_BG,
                fg=WARN_FG,
                font=self.fm(10, "bold"),
            ).grid(row=0, column=0, sticky="w", pady=(0, 5))
            tk.Label(
                tray_inner,
                text=self._tray_message(tracking),
                bg=WARN_BG,
                fg=WARN_FG,
                font=self.fb(11),
                justify="left",
        ...
        if self.show_details_var.get():
            ...
            offset_ok = result.offset_v is not None and (
                not OFFSET_GATE_ENABLED or OFFSET_MIN_V <= result.offset_v <= OFFSET_MAX_V
            )
            gated = tracking is not None and tracking.gate_enabled
            pending_accent = None if gated else WARN_ACCENT
            legacy_low = None if tracking is None else tracking.legacy_low_mv
            legacy_high = None if tracking is None else tracking.legacy_high_mv
            ratio = None if tracking is None else tracking.ratio_corrected
            ...
            self._result_tile(
                tiles, 1, f"{frequency_label(LOW_FREQUENCY_HZ)} (equiv.)", legacy_low,
                tracking is not None and tracking.low_outcome == OUTCOME_PASS,
                unit=" mV", decimals=1, accent_override=pending_accent,
            )
            self._result_tile(
                tiles, 2, f"{frequency_label(HIGH_FREQUENCY_HZ)} (equiv.)", legacy_high,
                tracking is not None and tracking.high_outcome == OUTCOME_PASS,
                unit=" mV", decimals=1, accent_override=pending_accent,
            )
            self._result_tile(
                tiles, 3, "Ratio 18/5 Hz", ratio,
                tracking is not None and tracking.ratio_outcome == OUTCOME_PASS,
                unit="", decimals=3, accent_override=pending_accent,
            )
            self._result_tile(tiles, 4, "Polarity", pol_verdict or None, pol_verdict == "GOOD")
            next_row += 1

            detail_bits = [f"Filter: {self.filter_setup}"]
            if tracking is not None:
                if tracking.raw_low_mv is not None:
                    detail_bits.append(
                        f"raw {frequency_label(LOW_FREQUENCY_HZ)} {tracking.raw_low_mv:.3f} mV "
                        f"(×{tracking.factor_low:.3f})"
                    )
                else:
                    detail_bits.append(f"{frequency_label(LOW_FREQUENCY_HZ)} not measured")
                if tracking.raw_high_mv is not None:
                    detail_bits.append(
                        f"raw {frequency_label(HIGH_FREQUENCY_HZ)} {tracking.raw_high_mv:.3f} mV "
                        f"(×{tracking.factor_high:.3f})"
                    )
                else:
                    detail_bits.append(f"{frequency_label(HIGH_FREQUENCY_HZ)} not measured")
                if tracking.ratio_raw is not None:
                    detail_bits.append(f"raw ratio {tracking.ratio_raw:.3f}")
                if gated:
                    detail_bits.append(
                        f"TP443 limits {tracking.min_legacy_low_mv:.0f} mV @ 5 Hz / "
                        f"{tracking.min_legacy_high_mv:.0f} mV @ 18 Hz / ratio "
                        f"{tracking.ratio_min:.2f}-{tracking.ratio_max:.2f}"
                    )
                    detail_bits.append(f"calibration {tracking.calibration_id}")
                else:
                    detail_bits.append("TP443 limits NOT applied (calibration pending)")
                if tracking.polarity_high:
                    detail_bits.append(
                        f"polarity {tracking.polarity_low or '—'} @ 5 Hz / "
                        f"{tracking.polarity_high} @ 18 Hz"
                    )
                elif result.polarity and polarity_good_bad(result.polarity):
                    detail_bits.append(f"polarity {result.polarity}")
                for label, snr_db in (
                    (frequency_label(LOW_FREQUENCY_HZ), tracking.snr_low_db),
                    (frequency_label(HIGH_FREQUENCY_HZ), tracking.snr_high_db),
                ):
                    if snr_db is not None and math.isfinite(snr_db):
                        detail_bits.append(f"SNR {snr_db:.1f} dB @ {label}")
                for label, measured_hz in (
                    (frequency_label(LOW_FREQUENCY_HZ), tracking.measured_low_hz),
                    (frequency_label(HIGH_FREQUENCY_HZ), tracking.measured_high_hz),
                ):
                    if measured_hz is not None and math.isfinite(measured_hz):
                        detail_bits.append(f"sync {measured_hz:.3f} Hz @ {label}")
            pol_detail = format_polarity_detail(self.last_metrics)
            if pol_detail:
                detail_bits.append(pol_detail)
            for label, report in (
                (frequency_label(LOW_FREQUENCY_HZ), self.last_capture_report),
                (frequency_label(HIGH_FREQUENCY_HZ), self.last_capture_report_high),
            ):
                if report is None:
                    continue
                if report.data_source == "simulator" and "SIMULATED DATA" not in detail_bits:
                    detail_bits.append("SIMULATED DATA")
                if report.capture_cycles:
                    detail_bits.append(
                        f"{label}: PWM on {report.pwm_on_seconds:.1f} s / {report.capture_cycles} cycles"
                    )
                if report.stabilized:
                    detail_bits.append(
                        f"{label}: stable at {report.stabilization_seconds:.1f} s / cycle "
                        f"{report.stabilization_cycle}, attempt {report.measurement_attempt}, "
                        f"window {report.measurement_cycles} cycles"
                    )
                elif report.unstable:
                    detail_bits.append(f"{label}: unstable on attempt {report.measurement_attempt}")
        ...
        self.btn(tools, "Capture waveforms", self.capture_waveform_snapshot, kind="ghost", size="sm").grid(row=0, column=1, padx=(0, 10))
        ToggleSwitch(
            tools,
            "Show test details",
    ...
    def _calibration_pending_message(self) -> str:
        tracking = self.last_tracking_report
        readings = ""
        if tracking is not None and tracking.raw_low_mv is not None and tracking.raw_high_mv is not None:
            readings = (
                f"Raw readings: {tracking.raw_low_mv:.3f} mV at {frequency_label(LOW_FREQUENCY_HZ)}, "
                f"{tracking.raw_high_mv:.3f} mV at {frequency_label(HIGH_FREQUENCY_HZ)}, raw ratio "
                f"{tracking.ratio_raw:.3f}. "
            )
        return (
            f"{readings}The per-frequency fixture factors have not been derived yet, so the "
            f"TP443 limits (1.2 V at 5 Hz, 0.72 V at 18 Hz, ratio 0.70-1.30) are not applied. "
            "This PASS only means the part gave a stable, positive response at both "
            "frequencies. Every reading is recorded in the batch CSV for the paired "
            "comparison against the legacy fixture."
        )

    def _tray_message(self, tracking: FrequencyTrackingReport) -> str:
        ratio_text = (
            f"corrected 18/5 Hz ratio {tracking.ratio_corrected:.3f}"
            if tracking.ratio_corrected is not None
            else "a spec 1-3 failure"
        )
        return (
            f"This sampled part reads {ratio_text}. TP443 spec 4: if one piece of the sample "
            f"fails specs 1-3, or reads {tracking.tray_ratio_threshold:.2f} or less on the ratio, "
            "measure the whole tray 100 %. Only parts that fail specs 1-3 themselves are failures."
        )

    def _build_result_banner(self, row: int, outcome: str, pending: bool = False) -> None:
        if outcome == OUTCOME_PASS and pending:
            accent, banner_bg, banner_fg = PASS_ACCENT, PASS_BG, PASS_FG
            glyph, verdict = "✓", "PASS · CALIBRATION PENDING"
        ...
        verdict_font = self.fd(38 if len(verdict) <= 6 else 24)
        ...
        def redraw() -> None:
            ...
            center_x = width / 2
            text_width = tkfont.Font(font=verdict_font).measure(verdict)
            glyph_x = center_x - text_width / 2 - S(48)
            banner.create_text(glyph_x, height / 2, text=glyph, fill=accent, font=self.fd(glyph_size))
            text_x = glyph_x + S(48) + S(26) * (1.0 - vals["text"])
            text_color = mix_color(banner_bg, banner_fg, vals["text"])
            banner.create_text(text_x, height / 2, anchor="w", text=verdict, fill=text_color, font=verdict_font)
    ...
    def _build_wave_canvas(self, row: int, live: bool) -> None:
        ...
        if live:
            ...
            tk.Label(
                wrapper,
                text=(
                    f"{frequency_label(LOW_FREQUENCY_HZ).upper()} / {EMITTER_PWM_DUTY_CYCLE:g}% DRIVEN CAPTURE  ·  "
                    "ADS AIN0 SENSOR + PWM SYNC OVERLAY (HIGH = EMITTER ON)"
                ),
        ...
        scope = ScopeView(
            ...
            empty_text=(
                ...
                if live
                else "NO 5 HZ CAPTURE — THE TEST ENDED BEFORE THE FIRST DRIVE"
            ),
        )
        ...
        # Result view: the second drive's capture on its own scope, so both
        # TP443 readings (and the polarity at each) can be verified visually.
        self.wave_canvas_high = None
        metrics_high = self.last_metrics_high
        if not live:
            wrapper.rowconfigure(3, weight=1)
            tk.Label(
                wrapper,
                text=(
                    f"{frequency_label(HIGH_FREQUENCY_HZ).upper()} / {EMITTER_PWM_DUTY_CYCLE:g}% DRIVEN CAPTURE  ·  "
                    "ADS AIN0 SENSOR + PWM SYNC OVERLAY (HIGH = EMITTER ON)"
                ),
                bg=PAGE_BG, fg=MUTED_FG, font=self.fm(10, "bold"),
            ).grid(row=2, column=0, columnspan=2, sticky="w", pady=(S(12), 0))
            high_scope = ScopeView(
                wrapper,
                self.animator,
                "step:scope_high",
                height=240,
                sample_rate_hz=(
                    metrics_high.sample_rate_hz if metrics_high is not None else DEFAULT_SAMPLE_RATE_HZ
                ),
                empty_text="NO 18 HZ CAPTURE — SKIPPED OR THE TEST ENDED BEFORE THE SECOND DRIVE",
            )
            high_scope.grid(row=3, column=0, columnspan=2, sticky="nsew", pady=(S(6), 0))
            self.wave_canvas_high = high_scope
            if metrics_high is not None and metrics_high.waveform_v.size >= 2:
                high_scope.set_data(metrics_high.waveform_v, metrics_high.sync_v)
    ...
    def update_navigation_state(self) -> None:
        ...
        if self.last_result is not None and not self.result_saved:
            ...
            self._set_footer_text(
                self.primary_button,
                "Measure again (Enter)" if self.last_measure_error else "Measure (Enter)",
            )
            self.primary_button.configure(state="normal" if idle else "disabled")
        self._fit_footer()
    ...
    def go_next(self) -> None:
        ...
        stopped or faulted attempt) it just reads again.
        """
    ...
    def abort_measurement(self) -> None:
        ...
        chunk boundary and raise, so the worker's own finally-blocks stop the
        stream and switch the emitter off, and every callback it posts is
        ...
        *,
        ...
        try:
            attempt_history.append_attempt(
                ...
                noise_worst_pp_mv=None,  # no emitter-off noise test in TP443
    ...
    # ----- record a sensor that could not be measured at all ----- #
    def can_record_not_measured(self) -> bool:
        """Only offered when an attempt recorded nothing."""
        return (
    ...
    def open_not_measured_window(self) -> None:
        if not self.can_record_not_measured():
            return
        dialog = tk.Toplevel(self)
        dialog.title(f"{OUTCOME_NOT_MEASURED}: {self.current_sensor_id}")
        ...
        frame.columnconfigure(0, weight=1)
        tk.Label(frame, text=f"{OUTCOME_NOT_MEASURED} —", bg=PAGE_BG, fg=ELTEC_RED, font=self.fm(11, "bold")).grid(row=0, column=0, sticky="w")
        tk.Label(
            frame,
        ...
        tk.Label(
            ...
            text=(
                ...
                f"It is not a pass, so the next part is still tested as "
                f"{self.current_sensor_id}."
            ),
            bg=PAGE_BG,
            fg=MUTED_FG,
            font=self.fb(12),
        ...
        def commit(end_batch: bool) -> None:
            if self.save_not_measured_sensor(reason_var.get(), note.get("1.0", "end-1c")):
                dialog.destroy()
                if end_batch:
                    self._end_batch()
                else:
        ...
        self.btn(buttons, "Record + Stop batch", lambda: commit(True), kind="outline", size="sm").grid(row=0, column=1, padx=(0, 10))
        self.btn(buttons, "Record + Next sensor", lambda: commit(False), kind="primary", size="sm").grid(row=0, column=2)
        reason_combo.focus_set()

    def save_not_measured_sensor(self, reason: str, note: str) -> bool:
    ...
    def save_current_sensor(self) -> bool:
        ...
        try:
            # Preserve every unstable capture (either drive) as a diagnostic
            # bundle before the verdict row is written.
            for frequency_hz, metrics, report, saved_attr in (
                (LOW_FREQUENCY_HZ, self.last_metrics, self.last_capture_report, "stability_diagnostics_saved"),
                (HIGH_FREQUENCY_HZ, self.last_metrics_high, self.last_capture_report_high, "stability_diagnostics_high_saved"),
            ):
                if report is None or not report.unstable or getattr(self, saved_attr):
                    continue
                if metrics is None:
                    raise RuntimeError(
                        f"The unstable {frequency_label(frequency_hz)} waveform is unavailable; "
                        "the result was not saved."
                    )
                suffix = f"unstable_{int(round(frequency_hz))}hz"
                diagnostic_paths = save_waveform_diagnostic_bundle(
                    ...
                    report,
                    title=(
                        f"{MODEL_NAME} {self.current_sensor_id} unstable at {frequency_label(frequency_hz)}"
                    ),
            ...
            append_result_csv(
                ...
                tracking_report=self.last_tracking_report,
                capture_report=self.last_capture_report,
                capture_report_high=self.last_capture_report_high,
    ...
    def run_measurement(self, _event: tk.Event | None = None) -> None:
        ...
        self._reset_measure_progress()
        self.measure_progress_total = 4 if REFERENCE_GATE_ENABLED else 3
        self._apply_preview_display_mode()
        self.measure_token += 1
        token = self.measure_token
        self.render_step()
    ...
    def _finish_drive(
        ...
        *,
        ...
        waveform_range_v: float,
        data_source: str,
    ) -> tuple[WaveformMetrics, StabilityCaptureReport, FinalResult | None]:
        """Turn one drive's adaptive capture into metrics + telemetry.

        Returns ``(metrics, capture_report, timeout_result)`` where
        ``timeout_result`` is the diagnostic FAIL when the capture never
        stabilized (``metrics`` then carries the transient for the scope) and
        None when the measurement window completed.
        """
        waveform, sync, actual_rate, stability_analysis, host_pwm_on_seconds = capture
        if stability_analysis.report.measurement_complete:
            ...
            metrics, timeout_result = build_stability_timeout_result(
                waveform,
                sync,
                actual_rate,
                stability_analysis,
            ...
            raise Esp32BackendError(
                f"Adaptive {frequency_label(frequency_hz)} capture ended without a complete result."
            )
        ...
        return metrics, report, timeout_result

    def _store_measurement(
        self,
        *,
        metrics_low: WaveformMetrics | None,
        report_low: StabilityCaptureReport | None,
        metrics_high: WaveformMetrics | None,
        report_high: StabilityCaptureReport | None,
        tracking: FrequencyTrackingReport | None,
    ) -> None:
        """Keep both drives' results for the result screen, CSV and snapshots.

        ``last_metrics`` (the 5 Hz capture) is set by on_measure_done from the
        returned metrics; everything else lives here.
        """
        self.last_capture_report = report_low
        self.last_metrics_high = metrics_high
        self.last_capture_report_high = report_high
        self.last_tracking_report = tracking
    ...
    def _hardware_measurement(self, filter_setup, waveform_range_v, pwm_channel, pwm_hz, pwm_duty, show_live, token, push):
        ...
        # TP443 per-part sequence: offset read (emitter off) -> reference gate
        # (while REFERENCE_GATE_ENABLED) -> 5 Hz / 20 % capture -> 18 Hz / 20 %
        # capture -> settled offset re-read -> ratio. The step ladder below
        # drives the measuring screen's progress bar.
        reference_steps = 1 if REFERENCE_GATE_ENABLED else 0
        low_step = 2 + reference_steps
        high_step = low_step + 1
        total_steps = high_step
        low_label = f"{frequency_label(pwm_hz)} sensitivity"
        high_label = f"{frequency_label(EMITTER_PWM_HIGH_FREQUENCY_HZ)} sensitivity"
        ...
        def offset_fail_fast(offset_value: float):
            """Record an immediate offset FAIL - nothing else is measured."""
            ...
            self._store_measurement(
                metrics_low=None, report_low=None, metrics_high=None,
                report_high=None, tracking=None,
            )
        ...
        with self.hardware_lock:
            ...
            # STEP 1: the offset read, deliberately BEFORE the reference gate.
            # The emitter is off and this is a plain DC read, so nothing about
            # the fixture needs to be trusted yet.
            ...
            # near 0 V. A high reading is never "no sensor". A just-inserted
            # part can also sit below the floor for a few seconds while its
            # offset wakes up, so poll briefly before declaring the slot empty.
            ...
            if offset_v < SENSOR_OFFSET_MIN_PLAUSIBLE_V:
                raise NoSensorDetectedError(
                    f"No sensor detected: AIN0 reads {offset_v:.3f} V DC, but a connected 449 M18 "
            ...
            # The early gate (while enabled) is HIGH-SIDE ONLY: these parts'
            # offsets settle upward after insertion, so a low first read is
            # not a verdict - the part continues and the offset is re-verified
            # after both captures. A HIGH first read is real and fails on the
            # spot without spending the capture time.
            if OFFSET_GATE_ENABLED and offset_v > OFFSET_MAX_V:
                return offset_fail_fast(offset_v)
            if OFFSET_GATE_ENABLED and offset_v < OFFSET_MIN_V:
            ...
            elif not OFFSET_GATE_ENABLED:
                push(lambda v=offset_v: self.set_measure_status(
                    token,
                    f"Offset {v:.3f} V recorded (band not gated - TP443 offset limits pending).",
                ))
            ...
            if REFERENCE_GATE_ENABLED:
                ...
                try:
                    def reference_gate_capture(attempt: int) -> float:
                        ...
                        # its qualified 10 Hz / 50 % drive, then switch to the
                        # TP443 drives for the DUT captures below.
                ...
                def high_offset_recheck():
                    ...
                    A part that has drifted above the offset limit (or railed)
                    explains the AIN1 failure: record the part as an immediate
                    high-offset FAIL and leave the reference calibration intact.
                    Only meaningful while the offset band is gated.
                    """
                    recheck_v = device.read_offset_voltage(waveform_range_v=waveform_range_v)
                    if not (
                        OFFSET_GATE_ENABLED
                        and high_offset_dut_explains_reference_failure(recheck_v)
                    ):
                    ...
                    return recheck_v, offset_fail_fast(recheck_v)
                ...
                push(lambda: self.set_measure_status(
                    token,
                    "Offset read. Reference gate is disabled (op-amp crosstalk) — "
            ...
            def drive_capture(frequency_hz: float, step: int, label: str):
                """One TP443 drive: PWM on at (frequency, 20 %), adaptive capture, PWM off.

                Returns ``(waveform, sync, rate, analysis, host_pwm_on_seconds)``.
                The command may reach the ESP32 before its acknowledgement
                fails, so PWM shutdown covers activation as well as the
                subsequent stream capture.
                """
                emitter_on_time: float | None = None
                emitter_off_time: float | None = None
                drive_text = f"{frequency_label(frequency_hz)} / {pwm_duty:g}%"
                cycles_required = measurement_cycles_for(frequency_hz)

                def progress(current: StabilityAnalysis) -> None:
                    current_report = current.report
                    if current_report.stabilized:
                        text = (
                            f"{drive_text} attempt {current_report.measurement_attempt}/{MAX_MEASUREMENT_ATTEMPTS}: "
                            f"stable at {current_report.stabilization_elapsed_s:.1f} s. "
                            f"Measuring cycle {current_report.measurement_cycle_count}/"
                    ...
                    push(lambda value=text: self.set_measure_status(token, value))
                    step_progress(step, label, fraction)
                ...
                def driven_capture(attempt: int):
                    ...
                    push(lambda: self.set_measure_status(
                        token,
                        f"Emitter PWM on at {drive_text}. Attempt 1/{MAX_MEASUREMENT_ATTEMPTS}: "
                        f"stabilizing peak (0/{DUT_STABILITY_CONFIRMATION_DELTAS})...",
                    ))
                    step_progress(step, label, 0.0)
                    return device.read_waveform_until_stable(
                        ...
                        sample_rate_hz=DEFAULT_SAMPLE_RATE_HZ,
                        expected_frequency_hz=frequency_hz,
                        measurement_cycles=cycles_required,
                        sync_frequency_tolerance_hz=sync_tolerance_for(frequency_hz),
                        progress=progress,
                        preview=preview,
                        cancelled=lambda: token != self.measure_token,
                    )
                ...
                def driven_capture_retry(attempt: int, exc: Exception) -> None:
                    push(lambda: self.set_measure_status(
                        token,
                        f"{stream_glitch_label(exc)} during the {drive_text} capture; "
                        f"nothing was recorded — restarting the capture "
                        f"({attempt}/{REFERENCE_READING_STREAM_RETRIES})…",
                    ))
                    push(lambda: self._log_stream_retry(
                        f"{drive_text} capture", attempt, exc
                    ))
                ...
                step_progress(step, label, 1.0)
                return waveform, sync, actual_rate, stability_analysis, host_pwm_on_seconds

            # STEP: 5 Hz / 20 % (TP443 spec 1), then 18 Hz / 20 % (spec 2).
            low_capture = drive_capture(pwm_hz, low_step, low_label)
            high_capture = None
            if low_capture[3].report.measurement_complete:
                push(lambda: self.set_measure_status(
                    token,
                    f"{frequency_label(pwm_hz)} capture complete. Switching the emitter to "
                    f"{frequency_label(EMITTER_PWM_HIGH_FREQUENCY_HZ)}…",
                ))
                high_capture = drive_capture(
                    EMITTER_PWM_HIGH_FREQUENCY_HZ, high_step, high_label
                )
            else:
                # Fail fast: a part that never stabilizes at 5 Hz is a FAIL
                # (TP443 note 1: remove and retest); do not spend the 18 Hz
                # deadline on it as well.
                push(lambda: self.set_measure_status(
                    token,
                    f"{frequency_label(pwm_hz)} capture did not stabilize — skipping the "
                    f"{frequency_label(EMITTER_PWM_HIGH_FREQUENCY_HZ)} capture and recording the failure…",
                ))

            # Settled-offset verification: re-read AIN0 now that the part has
            # been powered in the fixture through both captures. The VERDICT
            # and the CSV use this settled value; the insertion-time read is
            # recorded separately as offset_initial_v.
            offset_v = device.read_offset_voltage(waveform_range_v=waveform_range_v)

        push(lambda v=offset_v: self.on_offset_update(token, v))
        metrics_low, report_low, _timeout_low = self._finish_drive(
            low_capture, pwm_hz,
            offset_v=offset_v, waveform_range_v=waveform_range_v,
            data_source="esp32_449m18",
        )
        metrics_high: WaveformMetrics | None = None
        report_high: StabilityCaptureReport | None = None
        if high_capture is not None:
            metrics_high, report_high, _timeout_high = self._finish_drive(
                high_capture, EMITTER_PWM_HIGH_FREQUENCY_HZ,
                offset_v=offset_v, waveform_range_v=waveform_range_v,
                data_source="esp32_449m18",
            )
        final, tracking = evaluate_tracking_result(
            offset_v, metrics_low, metrics_high, filter_setup
        )
        if high_capture is None:
            final.warnings.append(
                f"{frequency_label(EMITTER_PWM_HIGH_FREQUENCY_HZ)} capture skipped: the "
                f"{frequency_label(pwm_hz)} capture did not stabilize."
            )
        self._store_measurement(
            metrics_low=metrics_low, report_low=report_low,
            metrics_high=metrics_high, report_high=report_high,
            tracking=tracking,
        )
        return metrics_low, final, offset_v
    ...
    def _simulate_measurement(self, filter_setup, sim_case, sim_low_battery, waveform_range_v, show_live, token, push):
        ...
        # One offset per simulated part, shared by both drives.
        stable_case = sim_case if sim_case in SIM_CASES and sim_case != "Never stabilizes" else "Known good"
        offset_v = simulate_offset_v(stable_case)
        push(lambda v=offset_v: self.on_initial_offset(token, v))
        push(lambda v=offset_v: self.on_offset_update(token, v))
        step_progress(1, "Offset", 1.0)

        # Mirror the hardware fail-fast, which is HIGH-SIDE ONLY and only
        # while the offset band is gated: the "High offset" case rails at the
        # ADC full scale like a real bad part.
        if OFFSET_GATE_ENABLED and offset_v > OFFSET_MAX_V:
            push(lambda v=offset_v: self.set_measure_status(
                token,
                f"Offset {v:.3f} V is outside the {OFFSET_MIN_V:.1f}-"
        ...
        def simulate_drive(frequency_hz: float, step: int, label: str):
            waveform, sync, actual_rate, _offset = simulate_v6_startup_capture(
                filter_setup,
                sim_case,
                frequency_hz=frequency_hz,
                offset_v=offset_v,
            )
            blocks_required = measurement_blocks_for(frequency_hz)
            push(lambda: self.set_measure_status(
                token,
                f"Emitter PWM on at {frequency_label(frequency_hz)} / {EMITTER_PWM_DUTY_CYCLE:g}% "
                "(simulated). Evaluating startup peak drift...",
            ))
            step_progress(step, label, 0.0)
            full_analysis = analyze_stability(
                waveform,
                stability_sync_for(sync, frequency_hz),
                actual_rate,
                dut_settings,
                stability_deadline_s=STABILITY_TIMEOUT_S,
                measurement_cycles_required=blocks_required,
            ...
            if stability_analysis.report.measurement_complete:
                push(lambda: self.set_measure_status(
                    token,
                    f"{frequency_label(frequency_hz)}: stable at "
                    f"{stability_analysis.report.stabilization_elapsed_s:.1f} s. Measured "
                    f"{stability_analysis.report.measurement_cycles_required} cycles on attempt "
                    f"{stability_analysis.report.measurement_attempt}.",
                ))
            metrics, report, timeout_result = self._finish_drive(
                (waveform, sync, actual_rate, stability_analysis, None),
                frequency_hz,
                offset_v=offset_v,
                waveform_range_v=waveform_range_v,
                data_source="simulator",
        ...
        metrics_low, report_low, timeout_low = simulate_drive(
            LOW_FREQUENCY_HZ, low_step, f"{frequency_label(LOW_FREQUENCY_HZ)} sensitivity"
        )
        metrics_high: WaveformMetrics | None = None
        report_high: StabilityCaptureReport | None = None
        if timeout_low is None:
            metrics_high, report_high, _timeout_high = simulate_drive(
                HIGH_FREQUENCY_HZ, high_step, f"{frequency_label(HIGH_FREQUENCY_HZ)} sensitivity"
            )
        final, tracking = evaluate_tracking_result(
            offset_v, metrics_low, metrics_high, filter_setup
        )
        if timeout_low is not None:
            final.warnings.append(
                f"{frequency_label(HIGH_FREQUENCY_HZ)} capture skipped: the "
                f"{frequency_label(LOW_FREQUENCY_HZ)} capture did not stabilize."
            )
        self._store_measurement(
            metrics_low=metrics_low, report_low=report_low,
            metrics_high=metrics_high, report_high=report_high,
    ...
    def _apply_preview_display_mode(self) -> None:
        """Keep the live scope in absolute-volts mode with the drive caption."""
    ...
    def on_measure_done(self, token: int, metrics: WaveformMetrics, final: FinalResult) -> None:
        ...
        tracking = self.last_tracking_report
        if outcome == OUTCOME_PASS and is_calibration_pending(final):
            ratio_text = (
                f" Raw ratio 18/5 Hz = {tracking.ratio_raw:.3f}."
                if tracking is not None and tracking.ratio_raw is not None
                else ""
            )
            self.status_var.set(
                f"{self.current_sensor_id}: PASS — stable positive response at both frequencies; "
                f"TP443 limits not applied (calibration pending).{ratio_text}"
            )
        elif outcome == OUTCOME_PASS and tracking is not None and tracking.tray_100_percent:
            self.status_var.set(
                f"{self.current_sensor_id}: {OUTCOME_PASS} — ratio "
                f"{tracking.ratio_corrected:.3f} is at or below {tracking.tray_ratio_threshold:.2f}: "
                "measure the whole tray (TP443 spec 4)."
            )
        elif outcome == OUTCOME_PASS:
            self.status_var.set(
    ...
    def on_reference_block(self, token: int, exc: ReferenceGateError) -> None:
        ...
        # re-check all sit in the message, and this row is the only place
        # they survive once the technician closes the warning - the
        # invalidated calibration file keeps the reading but not the part.
    ...
    def record_bad_sensor(self, offset_v: float) -> None:
        ...
        self._log_attempt(attempt_history.EVENT_MEASURED, result=final)
        self.status_var.set(
            f"{self.current_sensor_id}: FAIL — no offset with a sensor loaded (bad part). "
            f"Next saves it and reads the replacement as {self.current_sensor_id} again."
        )
        self.measure_status_var.set("")
        self.write_autosave("measurement_complete")
        self.render_step()
    ...
    def redraw_waveform(self) -> None:
        ...
        if self.wave_canvas_high is not None and self.wave_canvas_high.winfo_exists():
            metrics_high = self.last_metrics_high
            if metrics_high is not None and metrics_high.waveform_v.size >= 2:
                self.wave_canvas_high.set_data(metrics_high.waveform_v, metrics_high.sync_v)
    ...
    def capture_waveform_snapshot(self) -> None:
        ...
        saved: list[Path] = []
        try:
            for frequency_hz, metrics, report in (
                (LOW_FREQUENCY_HZ, self.last_metrics, self.last_capture_report),
                (HIGH_FREQUENCY_HZ, self.last_metrics_high, self.last_capture_report_high),
            ):
                if metrics is None:
                    continue
                saved.extend(
                    save_waveform_diagnostic_bundle(
                        self.batch_number,
                        self.current_sensor_id,
                        metrics,
                        report,
                        title=(
                            f"{MODEL_NAME} {self.current_sensor_id} "
                            f"{frequency_label(frequency_hz)} waveform snapshot"
                        ),
                        detail_lines=snapshot_detail_lines(
                            self.batch_number,
                            self.current_sensor_id,
                        ...
                        filename_suffix=f"snapshot_{int(round(frequency_hz))}hz",
                    )
                )
        except Exception as exc:
        ...
        if not saved:
            ...
            return
        self.snapshot_paths.extend(saved)
        self.stability_diagnostics_saved = self.last_capture_report is not None
        self.stability_diagnostics_high_saved = self.last_capture_report_high is not None
        self.update_comment_snapshot_status()
        self.write_autosave("waveform_snapshot_saved")
        self.status_var.set(f"Saved waveform snapshot: {saved[0]}")
    ...
    def write_autosave(self, stage: str) -> None:
        ...
        payload = {
            ...
            "offset_initial_v": self.last_offset_initial_v,
            "low_hz": LOW_FREQUENCY_HZ,
            "high_hz": HIGH_FREQUENCY_HZ,
            "pwm_duty": EMITTER_PWM_DUTY_CYCLE,
            # Raw 5 Hz reading under the shared name; the explicit
            # per-frequency values, the ratio and the gates follow.
            "sensitivity_mv": None if self.last_result is None else self.last_result.sensitivity_mv,
            "sensitivity_5hz_raw_mv": None if tracking is None else tracking.raw_low_mv,
            "sensitivity_5hz_legacy_equivalent_mv": None if tracking is None else tracking.legacy_low_mv,
            "sensitivity_5hz_outcome": None if tracking is None else tracking.low_outcome,
            "sensitivity_18hz_raw_mv": None if tracking is None else tracking.raw_high_mv,
            "sensitivity_18hz_legacy_equivalent_mv": None if tracking is None else tracking.legacy_high_mv,
            "sensitivity_18hz_outcome": None if tracking is None else tracking.high_outcome,
            "ratio_18_over_5_raw": None if tracking is None else tracking.ratio_raw,
            "ratio_18_over_5_corrected": None if tracking is None else tracking.ratio_corrected,
            "ratio_outcome": None if tracking is None else tracking.ratio_outcome,
            "ratio_tray_100_percent_flag": None if tracking is None else tracking.tray_100_percent,
            "sensitivity_factors": {
                f"{LOW_FREQUENCY_HZ:g}": sensitivity_factor(LOW_FREQUENCY_HZ),
                f"{HIGH_FREQUENCY_HZ:g}": sensitivity_factor(HIGH_FREQUENCY_HZ),
            },
            "sensitivity_calibration_id": SENSITIVITY_CALIBRATION_ID,
            "sensitivity_gate_enabled": SENSITIVITY_GATE_ENABLED,
            "polarity": None if self.last_result is None else self.last_result.polarity,
            "polarity_18hz": None if tracking is None else tracking.polarity_high,
    ...
    def show_batch_summary_window(self, batch_number: str, csv_path: Path) -> None:
        ...
        tray_flags = self._read_tray_flags(csv_path)
        ...
        if not SENSITIVITY_GATE_ENABLED:
            chip_specs.append(("TP443 LIMITS NOT APPLIED", WARN_FG, WARN_BG))
        ...
        if tray_flags:
            tk.Label(
                head,
                text=(
                    f"TP443 spec 4 — measure the whole tray 100 %: triggered by "
                    f"{attempt_history.format_sensor_ids(tray_flags)}"
                ),
                bg=PAGE_BG,
                fg="#8a5a00",
        ...
        columns = ("sensor", "offset", "low", "high", "ratio", "polarity", "result")
        headings = {
            "sensor": "Sensor",
            "offset": "Offset",
            "low": f"{frequency_label(LOW_FREQUENCY_HZ)} (equiv.)",
            "high": f"{frequency_label(HIGH_FREQUENCY_HZ)} (equiv.)",
            "ratio": "Ratio 18/5",
            "polarity": "Polarity",
            "result": "Result",
        }
        ...
        for column in columns:
            ...
            tree.column(column, width=130, anchor="center", stretch=True)
    ...
    def _read_tray_flags(self, csv_path: Path) -> list[str]:
        """Sensor ids whose verdict row raised the TP443 spec 4 tray flag."""
        flagged: list[str] = []
        try:
            with csv_path.open("r", newline="", encoding="utf-8") as csv_file:
                for row in csv.DictReader(csv_file):
                    if (row.get("ratio_tray_100_percent_flag") or "").strip().upper() == "YES":
                        flagged.append(row.get("sensor_id", ""))
        except Exception:
            return []
        return flagged

    def _read_summary_rows(self, csv_path: Path) -> list[tuple[str, str, str, str, str, str, str]]:
        rows: list[tuple[str, str, str, str, str, str, str]] = []
        try:
            with csv_path.open("r", newline="", encoding="utf-8") as csv_file:
                for row in csv.DictReader(csv_file):
                    low_equivalent = row.get("sensitivity_5hz_legacy_equivalent_mv", "")
                    low_raw = row.get("sensitivity_5hz_raw_mv", "") or row.get("sensitivity_mv", "")
                    high_equivalent = row.get("sensitivity_18hz_legacy_equivalent_mv", "")
                    high_raw = row.get("sensitivity_18hz_raw_mv", "")
                    ratio = (
                        row.get("ratio_18_over_5_corrected", "")
                        or row.get("ratio_18_over_5_raw", "")
                    )
                    rows.append(
                        (
                            row.get("sensor_id", ""),
                            self._fmt(row.get("offset_v", ""), 3, " V"),
                            self._summary_reading(low_equivalent, low_raw),
                            self._summary_reading(high_equivalent, high_raw),
                            self._fmt(ratio, 3, ""),
    ...
    def _summary_reading(self, equivalent: str, raw: str) -> str:
        if equivalent and raw and equivalent != raw:
            return f"{self._fmt(equivalent, 1, ' mV eq')} ({self._fmt(raw, 2, ' raw')})"
        return self._fmt(raw or equivalent, 2, " mV")
