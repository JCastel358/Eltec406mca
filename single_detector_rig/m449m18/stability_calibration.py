#!/usr/bin/env python3
"""Collect and summarize known-good data for the v6.1 stability threshold.
...
Capture uses the same peak/cycle extractor as the v6.1 application so that the
...
# 449 M18: 5 Hz / 18 Hz drives at 20 % duty (TP443 20/80 blade). 30 s gives
# 150 cycles at 5 Hz and 540 at 18 Hz - plenty for delta percentiles.
DEFAULT_CAPTURE_DURATION_S = 30.0
DEFAULT_FREQUENCY_HZ = LOW_FREQUENCY_HZ
DEFAULT_OUTPUT_DIR = (
    Path.home()
    / "Documents"
    / "Eltec_449M18_Test_Results"
    / "449m18_esp32"
    / "calibration"
)
...
# Keep these preflight limits aligned with the production 405 M22 rig without
# importing the Tk application (and therefore Tk/Matplotlib) into this CLI.
# Battery monitoring is DISABLED on this fixture (2026-08-12 battery isolation:
# 6.5 V battery drives the emitters only, 9 V battery drives the sensors, and
# neither is measurable on AIN7). TODO(hardware): re-enable once the sensor
# battery is measured on AIN6 through a >=4:1 divider.
BATTERY_MONITORING_ENABLED = False
BATTERY_MIN_V = 5.8
BATTERY_PLAUSIBLE_MIN_V = 3.0
BATTERY_PLAUSIBLE_MAX_V = 7.5
SENSOR_OFFSET_PLAUSIBLE_MIN_V = 0.05
SENSOR_OFFSET_PLAUSIBLE_MAX_V = 3.5   # firmware v2.0 reads the 0.8-3.0 V band linearly
...
class CaptureConfig:
    ...
    # Which TP443 drive to calibrate: 5 Hz or 18 Hz (both at 20 % duty).
    ...
    def __post_init__(self) -> None:
        ...
        if not any(
            math.isclose(frequency_hz, allowed, abs_tol=1e-6)
            for allowed in DUT_FREQUENCIES_HZ
        ):
            allowed_text = " or ".join(f"{value:g}" for value in DUT_FREQUENCIES_HZ)
            raise ValueError(f"frequency_hz must be {allowed_text} Hz (TP443 drives).")
...
def _validate_preflight(battery_v: float | None, offset_v: float) -> None:
    # battery_v is None while battery monitoring is disabled (nothing
    # measurable on AIN7 since the battery isolation) - only the offset half
    # of the preflight runs then.
...
def capture_calibration(
    ...
    *,
    ...
    try:
        ...
        # Establish a known-safe preflight state before reading the scalars.
        # The battery read is skipped while monitoring is disabled (nothing
        # measurable on AIN7 since the battery isolation).
        rig.disable_emitter_pwm("GPIO25")
        battery_v = (
            float(rig.read_battery_voltage()) if BATTERY_MONITORING_ENABLED else None
        )
        offset_v = float(rig.read_offset_voltage())
        _validate_preflight(battery_v, offset_v)

        # The backend returns the PWM,ON command timestamp after the PIN /
        # FREQ / DUTY setup. Test doubles/older compatible adapters fall back
        # to the first host instant after the acknowledged command.
        activation_time = rig.enable_emitter_pwm(
            "GPIO33", config.frequency_hz, PWM_DUTY_CYCLE_PERCENT
        )
    ...
    try:
        # TP443 counter tolerances: +/-0.05 Hz at 5 Hz, +/-0.18 Hz at 18 Hz
        # (mirrors the production tester's SYNC_FREQUENCY_TOLERANCE_HZ).
        sync_validation = validate_rising_sync_cycles(
            sync,
            actual_rate_hz,
            expected_frequency_hz=config.frequency_hz,
            frequency_tolerance_hz=0.01 * config.frequency_hz,
        )
    ...
    summary: dict[str, Any] = {
        ...
        "capture": {
            "drive_frequency_hz": config.frequency_hz,
            "drive_duty_percent": PWM_DUTY_CYCLE_PERCENT,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Collect and summarize 449 M18 (5 Hz / 18 Hz, 20 % duty) peak-delta "
            "calibration evidence from known-good sensors."
        )
    )
    ...
    capture.add_argument(
        ...
        metavar="HZ",
        help=(
            "TP443 drive to calibrate, 5 or 18 Hz at the fixed 20 %% duty "
            f"(default: {DEFAULT_FREQUENCY_HZ:g})."
        ),
    )
    capture.add_argument(
        "--output-dir",
