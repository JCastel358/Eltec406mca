...
import stability_calibration as calibration  # noqa: E402
from stability_analysis import StabilitySettings  # noqa: E402


def stable_stream(sample_count: int = 8002) -> list[backend.StreamSample]:
    """Return 5 Hz / 20 % samples with rising edges at 1, 201, 401, ..."""

    samples: list[backend.StreamSample] = []
    for index in range(sample_count):
        phase = index % 200
        sync = 1 if 1 <= phase <= 40 else 0
...
class FakeRig:
    ...
    def connect(self) -> None:
        ...
        self.identity = backend.FirmwareIdentity(
            "ELTEC-ESP32-ADS1256,v3.2.2", (3, 2, 2), True
        )
    ...
    def enable_emitter_pwm(
        self, _channel: str, frequency_hz: float = 5.0, duty_cycle_percent: float = 20.0
    ) -> None:
        self.events.append("pwm_on")
        self.drives = getattr(self, "drives", [])
        self.drives.append((float(frequency_hz), float(duty_cycle_percent)))
...
class CaptureTests(unittest.TestCase):
    def test_capture_writes_raw_cycles_and_summary_from_one_stream(self):
        rig = FakeRig()
        clock_values = iter((10.0, 10.003, 18.004))
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(len(raw_rows), 8002)
            self.assertEqual(raw_rows[0]["timestamp_us"], "0")
            self.assertEqual(raw_rows[-1]["elapsed_s"], "8.001000000")
            self.assertEqual(raw_rows[0]["pwm_elapsed_s"], "0.003000000")
            self.assertEqual(raw_rows[-1]["pwm_elapsed_s"], "8.004000000")
            # 8.001 s of 5 Hz drive = 40 complete 200-sample cycles.
            self.assertEqual(len(cycle_rows), 40)
            self.assertEqual(rig.drives, [(5.0, 20.0)])
            ...
            self.assertEqual(disk_summary["capture"]["drive_frequency_hz"], 5.0)
            self.assertEqual(disk_summary["capture"]["drive_duty_percent"], 20.0)
            self.assertEqual(
                disk_summary["hardware"]["firmware_identity"],
                "ELTEC-ESP32-ADS1256,v3.2.2",
            )
            self.assertTrue(disk_summary["calibration_only"])
            self.assertIsNone(disk_summary["hardware"]["battery_v"])
            self.assertNotIn("verdict", json.dumps(disk_summary).lower())

        # Battery monitoring is disabled: the preflight goes straight from the
        # safe PWM-off state to the offset read.
        self.assertEqual(rig.events[:3], ["connect", "pwm_off", "offset"])
        self.assertNotIn("battery", rig.events)
    ...
    def test_battery_is_never_read_and_offset_preflight_still_blocks(self):
        # Battery monitoring is disabled (nothing measurable on AIN7 since the
        # battery isolation): a rig reporting a dead battery is never asked
        # for it. The offset half of the preflight still blocks a bad rig
        # before PWM ever turns on.
        self.assertFalse(calibration.BATTERY_MONITORING_ENABLED)
        rig = FakeRig(battery_v=5.8, offset_v=3.9)
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(
                calibration.PreflightError, "outside the plausible sensor"
            ):
    ...
    def test_wrong_sync_frequency_is_not_written_as_calibration_evidence(self):
        ...
        for index, original in enumerate(stable_stream()):
            samples.append(
                backend.StreamSample(
                    ...
                    sync=1 if 1 <= (index % 100) <= 50 else 0,
                )
            )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # A 10 Hz sync against the requested 5 Hz drive is not evidence.
            with self.assertRaisesRegex(
...
class ParserTests(unittest.TestCase):
    def test_capture_parser_exposes_requested_interface_and_defaults(self):
        ...
        self.assertEqual(calibration.DEFAULT_OUTPUT_DIR.parent.name, "449m18_esp32")
        self.assertEqual(args.settings, calibration.DEFAULT_SETTINGS_PATH)

    def test_capture_config_accepts_only_the_two_tp443_drives(self):
        config = calibration.CaptureConfig(sensor_id="GOOD-1", frequency_hz=18)
        self.assertEqual(config.frequency_hz, 18.0)
        with self.assertRaisesRegex(ValueError, "5 or 18 Hz"):
            calibration.CaptureConfig(sensor_id="GOOD-1", frequency_hz=10.0)
        args = calibration.build_parser().parse_args(
            ["capture", "--sensor-id", "GOOD-1", "--frequency", "18"]
        )
        self.assertEqual(args.frequency, 18.0)
