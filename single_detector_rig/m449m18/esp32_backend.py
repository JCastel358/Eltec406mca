"""Serial backend for the Eltec ESP32 + ADS1256 rig - 449 M18 (5/18 Hz) build.

This module intentionally contains no Tkinter or NumPy dependencies.  It is
the small hardware boundary used by the GUI: pyserial discovers and opens the
board, this class validates the firmware protocol, and the GUI turns the
returned samples into NumPy arrays for the existing analysis engine.

The firmware protocol is defined in ``Arduino/Eltec/Eltec.ino``. This build
requires firmware v3.2 or newer: the 449 M18 frequency-tracking test (TP443)
drives the emitter at 5 Hz and then 18 Hz with a 20 % ON / 80 % OFF duty to
mimic the legacy fixture's 20/80 chopper blade, and ``PWM,DUTY,<pct>`` only
exists from v3.2 (every earlier build is hard-wired to 50 %). The board boots
at 10 Hz / 50 %, so this backend explicitly programs frequency AND duty
...
# PWM,DUTY,<pct> (v3.2+) before every PWM,ON. The permanently mounted AIN1
# reference unit is a 406MCA sensor, so reference phases drive the emitter at
# that model's qualified 10 Hz / 50 % instead.
LOW_FREQUENCY_HZ = 5.0
HIGH_FREQUENCY_HZ = 18.0
DUT_FREQUENCIES_HZ = (LOW_FREQUENCY_HZ, HIGH_FREQUENCY_HZ)
# Kept as the "default drive" name other modules expect (the first TP443
# measurement); the second capture passes HIGH_FREQUENCY_HZ explicitly.
PWM_FREQUENCY_HZ = LOW_FREQUENCY_HZ
REFERENCE_PWM_FREQUENCY_HZ = 10.0
PWM_DUTY_CYCLE_PERCENT = 20.0
REFERENCE_PWM_DUTY_CYCLE_PERCENT = 50.0
# Emitter gate. Moved D25 -> D33 on 2026-08-25 (firmware v3.1 boots on 33 too;
# the backend sends PIN,<n> after connect either way).
PWM_GPIO = 33
# v3.2 = PWM,DUTY,<pct>; without it the 20/80 drive cannot be produced, so
# older firmware is refused at connect (there is no silent 50 % fallback: a
# 50 % capture would not be comparable to the 20/80 calibration factors).
...
class Esp32Rig:
    ...
    def connect(self) -> None:
        """Connect and validate firmware v3.2 or newer (PWM,FREQ + PWM,DUTY).
    ...
    def _validate_identity(line: str, ready_seen: bool) -> FirmwareIdentity:
        ...
        if version < MINIMUM_FIRMWARE_VERSION:
            ...
            raise UnsupportedFirmwareError(
                ...
                "The 449 M18 build needs PWM,DUTY (v3.2) to drive the emitter "
                "at 5 Hz and 18 Hz with the 20/80 blade duty of the TP443 "
                "fixture; older firmware is fixed at 50 % and its readings "
                "would not match the calibration factors."
            )
    ...
    def _validate_fixed_pwm(
        ...
        # Exactly the qualified (frequency, duty) drives: the two TP443 DUT
        # frequencies at the 20/80 blade duty, and the 406MCA reference
        # unit's 10 Hz / 50 %. Anything else is rejected before serial IO.
        allowed_drives = (
            (LOW_FREQUENCY_HZ, PWM_DUTY_CYCLE_PERCENT),
            (HIGH_FREQUENCY_HZ, PWM_DUTY_CYCLE_PERCENT),
            (REFERENCE_PWM_FREQUENCY_HZ, REFERENCE_PWM_DUTY_CYCLE_PERCENT),
        )
        if not any(
            math.isclose(float(frequency_hz), allowed_hz, abs_tol=1e-6)
            and math.isclose(float(duty_cycle_percent), allowed_duty, abs_tol=1e-6)
            for allowed_hz, allowed_duty in allowed_drives
        ):
            raise ValueError(
                "The 449 M18 build drives the emitter at 5 Hz or 18 Hz with a "
                f"{PWM_DUTY_CYCLE_PERCENT:g}% duty (TP443 20/80 blade) for the DUT, "
                f"or 10 Hz / {REFERENCE_PWM_DUTY_CYCLE_PERCENT:g}% for the 406MCA "
                "reference unit; no other drive is a qualified production drive."
            )
    ...
    def configure_emitter_pwm(
        ...
        """Select the gate pin and enable a qualified (frequency, duty) drive.

        5 Hz / 20 % (default) and 18 Hz / 20 % are the TP443 DUT drives;
        10 Hz / 50 % drives the permanently mounted 406MCA reference unit at
        its qualified frequency.
        """

        self._validate_fixed_pwm(channel, frequency_hz, duty_cycle_percent)
        self._command(f"PIN,{PWM_GPIO}", f"OK,PIN,{PWM_GPIO}")
        # The board boots at 10 Hz / 50 %; program BOTH settings on every
        # activation so a board reset can never leave a test running at the
        # wrong chop frequency or duty.
        self._command(f"PWM,FREQ,{float(frequency_hz):g}", "OK,PWM,FREQ")
        self._command(f"PWM,DUTY,{float(duty_cycle_percent):g}", "OK,PWM,DUTY")
