#!/usr/bin/env bash
# Install 449 M18 XFCE desktop and per-user Applications-menu launchers.
...
LAUNCHER="$SCRIPT_DIR/run_eltec_449m18_esp32_tester.sh"
APP_PATH="$SCRIPT_DIR/eltec_449m18_esp32_tester.py"
ICON_PATH="$SCRIPT_DIR/assets/eltec_desktop_icon.png"
ENTRY_NAME='com.eltec.449m18-esp32-tester.desktop'
...
DESKTOP_ENTRY="$DESKTOP_DIR/Eltec 449 M18 ESP32 Tester.desktop"
...
TMP_ENTRY=$(mktemp "${TMPDIR:-/tmp}/eltec-449m18-launcher.XXXXXX.desktop")
...

{
    ...
    printf '%s\n' 'Version=1.0'
    printf '%s\n' 'Type=Application'
    printf '%s\n' 'Name=Eltec 449 M18 ESP32 Tester'
    printf '%s\n' 'GenericName=449 M18 Frequency Tracking Tester (5/18 Hz)'
    printf '%s\n' 'Comment=Test Eltec 449 M18 sensors at 5 Hz and 18 Hz (TP443 frequency tracking) with adaptive peak stabilization'
    printf 'Exec=%s\n' "$EXEC_VALUE"
    printf 'Icon=%s\n' "$ICON_VALUE"
    printf '%s\n' 'Terminal=false'
    printf '%s\n' 'Categories=Utility;'
...
printf 'Installed the Eltec 449 M18 ESP32 Tester launcher:\n'
