...
class ConnectionTests(unittest.TestCase):
    def test_auto_connect_validates_candidates_and_is_idempotent(self):
        ...
        factory = SerialFactory(
            {
                ...
                "/dev/ttyUSB1": lambda **_options: ScriptedSerial(
                    {"IDN?": ["ELTEC-ESP32-ADS1256,v2.0.2"]},
        ...
        self.assertEqual(rig.identity.version, (2, 0, 2))
    ...
    def test_connect_tolerates_absent_or_failing_set_buffer_size(self):
        ...
        for serial_port in (
            ScriptedSerial({"IDN?": ["ELTEC-ESP32-ADS1256,v2.0"]}),
            RejectingScriptedSerial({"IDN?": ["ELTEC-ESP32-ADS1256,v2.0"]}),
        ):
    ...
    def test_old_firmware_is_rejected_with_flash_guidance(self):
        # v1.9 still has the gain-2 buffered ADS1256 front end that clips the
        # TP412 0.8-3.0 V offset band at 2.5 V, and v1.8 additionally predates
        # PWM,FREQ - both must be rejected by the v2.0-only 405 M22 backend.
        for stale_version in ("v1.8", "v1.9"):
            with self.subTest(stale_version=stale_version):
                ...
                with self.assertRaisesRegex(
                    backend.UnsupportedFirmwareError, "flash v2.0"
                ):
                    rig.connect()
    ...
    def test_probe_contract_returns_bool_and_message(self):
        ...
        fake_rig.identity = backend.FirmwareIdentity(
            "ELTEC-ESP32-ADS1256,v2.0", (2, 0, 0), True
        )
        ...
        self.assertEqual(
            result,
            (True, "ELTEC-ESP32-ADS1256,v2.0 connected on /dev/ttyUSB0."),
        )
...
class ScalarAndPwmTests(unittest.TestCase):
    ...
    def test_scalar_reads_and_gui_compatible_pwm_signatures(self):
        ...
        activation_time = self.rig.configure_emitter_pwm("DIO0", 1.0, 50.0)
    ...
    def test_reference_pwm_at_ten_hz_programs_the_board(self):
        ...
        # the emitter at its qualified 10 Hz instead of the DUT's 1 Hz.
        self.rig.configure_emitter_pwm(
            "GPIO25", backend.REFERENCE_PWM_FREQUENCY_HZ, 50.0
        )
        self.assertTrue(self.rig.pwm_enabled)
        self.assertEqual(
    ...
    def test_non_fixed_pwm_is_rejected_before_hardware_command(self):
        ...
        # Only the DUT's 1 Hz and the reference unit's 10 Hz are qualified
        # production drives; anything else is rejected before any serial IO.
        with self.assertRaisesRegex(ValueError, "qualified production drive"):
            self.rig.configure_emitter_pwm("DIO0", 5.0, 50.0)
        with self.assertRaisesRegex(ValueError, "fixed at 50%"):
            self.rig.configure_emitter_pwm("GPIO25", 1.0, 40.0)
