@echo off
rem Launch the Windows/ESP32 405 M22 (1 Hz) tester from any working directory.
rem Windows counterpart of run_eltec_405m22_esp32_tester.sh - same app, same
rem results folder (%USERPROFILE%\Documents\Eltec_405M22_Test_Results).
setlocal EnableExtensions EnableDelayedExpansion

set "SCRIPT_DIR=%~dp0"
if "%SCRIPT_DIR:~-1%"=="\" set "SCRIPT_DIR=%SCRIPT_DIR:~0,-1%"
set "APP_PATH=%SCRIPT_DIR%\eltec_405m22_esp32_tester.py"

rem --- log file (mirrors the Xubuntu launcher's ~/.local/state location) ----
set "LOG_DIR=%LOCALAPPDATA%\eltec-405m22-esp32"
if not exist "%LOG_DIR%" mkdir "%LOG_DIR%" 2>nul
if not exist "%LOG_DIR%" set "LOG_DIR=%TEMP%\eltec-405m22-esp32"
if not exist "%LOG_DIR%" mkdir "%LOG_DIR%" 2>nul
if not exist "%LOG_DIR%" (
    echo Eltec 405 M22 ESP32 Tester: could not create a log directory. 1>&2
    exit /b 1
)
...
>>"%LOG_FILE%" 2>&1 (
    echo.
    echo [%DATE% %TIME%] Starting Eltec 405 M22 ESP32 Tester
...
echo Eltec 405 M22 ESP32 Tester: %MESSAGE% 1>&2
...
powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command ^
    "Add-Type -AssemblyName System.Windows.Forms; [void][System.Windows.Forms.MessageBox]::Show($env:MESSAGE + [Environment]::NewLine + [Environment]::NewLine + 'Details: ' + $env:LOG_FILE, 'Eltec 405 M22 tester could not start', 'OK', 'Error')" >nul 2>&1
goto :eof
