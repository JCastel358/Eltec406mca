@echo off
rem Launch the unified Eltec Test Rig sensor-version selector from any working
rem directory. Windows counterpart of run_eltec_rig_tester.sh - same app; each
rem sensor version keeps its own results folder (see README.md).
setlocal EnableExtensions EnableDelayedExpansion

set "SCRIPT_DIR=%~dp0"
if "%SCRIPT_DIR:~-1%"=="\" set "SCRIPT_DIR=%SCRIPT_DIR:~0,-1%"
set "APP_PATH=%SCRIPT_DIR%\eltec_rig_tester.py"

rem --- log file (mirrors the Xubuntu launcher's ~/.local/state location) ----
set "LOG_DIR=%LOCALAPPDATA%\eltec-rig"
if not exist "%LOG_DIR%" mkdir "%LOG_DIR%" 2>nul
if not exist "%LOG_DIR%" set "LOG_DIR=%TEMP%\eltec-rig"
if not exist "%LOG_DIR%" mkdir "%LOG_DIR%" 2>nul
if not exist "%LOG_DIR%" (
    echo Eltec Test Rig: could not create a log directory. 1>&2
    exit /b 1
)
...
>>"%LOG_FILE%" 2>&1 (
    echo.
    echo [%DATE% %TIME%] Starting Eltec Test Rig
...
echo Eltec Test Rig: %MESSAGE% 1>&2
...
powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command ^
    "Add-Type -AssemblyName System.Windows.Forms; [void][System.Windows.Forms.MessageBox]::Show($env:MESSAGE + [Environment]::NewLine + [Environment]::NewLine + 'Details: ' + $env:LOG_FILE, 'Eltec Test Rig could not start', 'OK', 'Error')" >nul 2>&1
goto :eof
