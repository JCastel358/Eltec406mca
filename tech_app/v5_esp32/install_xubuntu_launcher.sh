#!/usr/bin/env bash
# Install an XFCE desktop launcher and a per-user Applications-menu entry.
...
ENTRY_NAME='com.eltec.406mca-esp32-tester.desktop'
...
DESKTOP_ENTRY="$DESKTOP_DIR/Eltec 406MCA ESP32 Tester.desktop"
...
TMP_ENTRY=$(mktemp "${TMPDIR:-/tmp}/eltec-406mca-launcher.XXXXXX.desktop")
...

{
    ...
    printf '%s\n' 'Version=1.0'
    printf '%s\n' 'Type=Application'
    printf '%s\n' 'Name=Eltec 406MCA ESP32 Tester'
    printf '%s\n' 'GenericName=406MCA Emitter Tester'
    printf '%s\n' 'Comment=Test Eltec 406MCA emitters with the ESP32/ADS1256 rig'
    printf 'Exec=%s\n' "$EXEC_VALUE"
    printf 'Icon=%s\n' "$ICON_VALUE"
    printf '%s\n' 'Terminal=false'
    printf '%s\n' 'Categories=Utility;'
...
printf 'Installed the Eltec 406MCA ESP32 Tester launcher:\n'
