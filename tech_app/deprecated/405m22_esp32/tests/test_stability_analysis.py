...
class BandLimitedNoiseAnalysisTests(unittest.TestCase):
    ...
    def test_high_frequency_noise_is_removed_low_frequency_kept(self):
        ...
        # sample (500 Hz) averages away entirely; a ±200 µV window-centered
        # slow square survives at full amplitude through decimation AND the
        # per-window detrend.
        ...
        self.assertLess(fast_analysis.worst_pp_mv, 0.001)
        self.assertFalse(slow_analysis.passed)
        self.assertAlmostEqual(slow_analysis.worst_pp_mv, 0.400, delta=0.001)
    ...
    def test_raw_clipping_still_fails_a_window_the_average_would_hide(self):
        # One window rides at the +4.95 V rail: the boxcar average is a
        # quiet DC level (tiny filtered pk-pk), but the raw samples prove
        # the input railed, so the window must count as over-limit.
        quiet = [1.2] * 19_000
        clipped = [4.95] * 1000
