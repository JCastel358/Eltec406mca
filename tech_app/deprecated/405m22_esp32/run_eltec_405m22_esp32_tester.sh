#!/usr/bin/env bash
# Launch the Xubuntu/ESP32 405 M22 (1 Hz) tester from any working directory.
...
APP_PATH="$SCRIPT_DIR/eltec_405m22_esp32_tester.py"
PYTHON_BIN=${ELTEC_PYTHON:-python3}

STATE_ROOT=${XDG_STATE_HOME:-"$HOME/.local/state"}
LOG_DIR="$STATE_ROOT/eltec-405m22-esp32"
if ! mkdir -p -- "$LOG_DIR" 2>/dev/null; then
    LOG_DIR="${TMPDIR:-/tmp}/eltec-405m22-esp32-${UID}"
    mkdir -p -- "$LOG_DIR" || {
        printf 'Eltec 405 M22 ESP32 Tester: could not create a log directory.\n' >&2
        exit 1
    }
fi
...
show_error() {
    ...
    printf 'Eltec 405 M22 ESP32 Tester: %s\nDetails: %s\n' "$message" "$LOG_FILE" >&2

    if command -v notify-send >/dev/null 2>&1 \
        && notify-send --urgency=critical --app-name='Eltec 405 M22 ESP32 Tester' \
            'Eltec 405 M22 tester could not start' "$details" >/dev/null 2>&1; then
        return
    fi
    if command -v zenity >/dev/null 2>&1; then
        zenity --error --title='Eltec 405 M22 tester could not start' --text="$details" \
            >/dev/null 2>&1 || true
...
{
    printf '\n[%s] Starting Eltec 405 M22 ESP32 Tester\n' "$(date '+%Y-%m-%d %H:%M:%S')"
