...
# 405 M22: 1 Hz chop means each cycle takes a second - capture 60 s so a run
# still contains enough cycles for delta percentiles.
DEFAULT_CAPTURE_DURATION_S = 60.0
DEFAULT_OUTPUT_DIR = (
    Path.home()
    / "Documents"
...
def capture_calibration(
    ...
    *,
    ...
    try:
        ...
        # The v6.1 backend returns the PWM,ON command timestamp after any PIN
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Collect and summarize 405 M22 (1 Hz) peak-delta calibration "
            "evidence from known-good sensors."
        )
    )
