"""Peak-delta stabilization analysis for the 405 M22 (1 Hz) ESP32 tester.

Identical rules to the 406MCA v6/v6.1 build, retimed for the Model 405 M22's
1 Hz chop: each PWM cycle now lasts a full second, so the stabilization
deadline is 60 s instead of 20 s (a 5-delta confirmation plus measurement
cycles needs tens of seconds of wall time at 1 Hz).
...
# production DUT limit is the tracked ``stability_settings.json`` value
# (0.500 mV for the 405 M22); the AIN1 reference unit uses the application's
# own REFERENCE_PEAK_DELTA_THRESHOLD_MV instead of either of these.
DEFAULT_PEAK_DELTA_THRESHOLD_MV = 0.100
DEFAULT_CONSECUTIVE_DELTAS_REQUIRED = 5
# At 1 Hz every cycle takes 1 s: qualification alone needs >= 6 cycles and a
# measurement window 10 more, so the 406MCA's 20 s deadline is far too tight.
DEFAULT_STABILITY_DEADLINE_S = 60.0
DEFAULT_MEASUREMENT_CYCLES_REQUIRED = 10
DEFAULT_MAX_MEASUREMENT_ATTEMPTS = 3
# Model 405 M22 drive: 1 Hz +/- 0.1 Hz.
DEFAULT_SYNC_FREQUENCY_HZ = 1.0
...
def decimate_boxcar(
    ...
    A boxcar decimation is the band-limiting step of the noise analysis: it
    reduces white instrument noise by sqrt(factor) and rolls off content
    above ~0.443 * (rate/factor), approximating the low-frequency view the
    legacy band-limited test amplifier had. ``factor`` 1 is a pass-through.
    """
...
def analyze_noise_capture_band_limited(
    ...
    *,
    ...
    """Windowed pk-pk noise verdict on a boxcar-decimated capture.
    ...
    if not all(math.isfinite(value) for value in raw):
        raise ValueError("waveform samples must all be finite")
    filtered = decimate_boxcar(raw, decimation_factor)
