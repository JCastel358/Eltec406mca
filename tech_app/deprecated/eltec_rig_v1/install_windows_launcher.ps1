<#
.SYNOPSIS
    Install Desktop and Start Menu shortcuts for the Eltec Test Rig.
...
.DESCRIPTION
    ...
    rights are needed. Both shortcuts point at run_eltec_rig_tester.cmd in
    this folder, so moving or updating the checkout keeps working as long as
    the shortcuts are reinstalled.
...
$Launcher = Join-Path $ScriptDir 'run_eltec_rig_tester.cmd'
$AppPath = Join-Path $ScriptDir 'eltec_rig_tester.py'
...
try {
    foreach ($link in @($DesktopLink, $StartMenuLink)) {
        ...
        $shortcut.Description = 'Eltec sensor test rig - choose the sensor version (405 M22, 406 MCA, ...) and test on the ESP32/ADS1256 rig'
...
""
"Launcher : $Launcher"
"Log file : $env:LOCALAPPDATA\eltec-rig\launcher.log"
"Results  : $([Environment]::GetFolderPath('MyDocuments'))\Eltec_405M22_Test_Results (405 M22) and \Eltec_406MCA_Test_Results (406 MCA)"
