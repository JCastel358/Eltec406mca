#!/usr/bin/env bash
# Install failure-calibration XFCE desktop and Applications-menu launchers.
...
ENTRY_NAME='com.eltec.406mca-failure-cal-v6-1.desktop'
...
DESKTOP_ENTRY="$DESKTOP_DIR/Eltec 406MCA Failure Calibration — v6.1 Base.desktop"
...
TMP_ENTRY=$(mktemp "${TMPDIR:-/tmp}/eltec-406mca-failure-cal-v6-1-launcher.XXXXXX.desktop")
...

{
    ...
    printf '%s\n' 'GenericName=406MCA Verdict Calibration Study'
    printf '%s\n' 'Comment=Collect technician ground truth for experimental 406MCA verdict calibration'
    printf 'Exec=%s\n' "$EXEC_VALUE"
    printf 'Icon=%s\n' "$ICON_VALUE"
    printf '%s\n' 'Terminal=false'
    printf '%s\n' 'Categories=Utility;'
...
printf 'Installed the Eltec 406MCA Failure Calibration — v6.1 Base launcher:\n'
