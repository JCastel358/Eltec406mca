#!/usr/bin/env bash
# Launch the v6.1-based failure-calibration app from any working directory.
...
LOG_DIR="$STATE_ROOT/eltec-406mca-failure-cal-v6-1"
if ! mkdir -p -- "$LOG_DIR" 2>/dev/null; then
    LOG_DIR="${TMPDIR:-/tmp}/eltec-406mca-failure-cal-v6-1-${UID}"
    mkdir -p -- "$LOG_DIR" || {
        printf 'Eltec 406MCA Failure Calibration — v6.1 Base: could not create a log directory.\n' >&2
        exit 1
    }
fi
...
show_error() {
    ...
    printf 'Eltec 406MCA Failure Calibration — v6.1 Base: %s\nDetails: %s\n' "$message" "$LOG_FILE" >&2

    if command -v notify-send >/dev/null 2>&1 \
        && notify-send --urgency=critical --app-name='Eltec 406MCA Failure Calibration — v6.1 Base' \
            'Eltec failure calibration could not start' "$details" >/dev/null 2>&1; then
        return
    fi
    if command -v zenity >/dev/null 2>&1; then
        zenity --error --title='Eltec failure calibration could not start' --text="$details" \
            >/dev/null 2>&1 || true
...
{
    printf '\n[%s] Starting Eltec 406MCA Failure Calibration — v6.1 Base\n' "$(date '+%Y-%m-%d %H:%M:%S')"
