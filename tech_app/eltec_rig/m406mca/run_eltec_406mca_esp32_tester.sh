#!/usr/bin/env bash
# Launch the Xubuntu/ESP32 v6.1 tester from any working directory.
...
LOG_DIR="$STATE_ROOT/eltec-406mca-esp32-v6-1"
if ! mkdir -p -- "$LOG_DIR" 2>/dev/null; then
    LOG_DIR="${TMPDIR:-/tmp}/eltec-406mca-esp32-v6-1-${UID}"
    mkdir -p -- "$LOG_DIR" || {
        printf 'Eltec 406MCA ESP32 Tester v6.1: could not create a log directory.\n' >&2
        exit 1
    }
fi
...
show_error() {
    ...
    printf 'Eltec 406MCA ESP32 Tester v6.1: %s\nDetails: %s\n' "$message" "$LOG_FILE" >&2

    if command -v notify-send >/dev/null 2>&1 \
        && notify-send --urgency=critical --app-name='Eltec 406MCA ESP32 Tester v6.1' \
            'Eltec v6.1 tester could not start' "$details" >/dev/null 2>&1; then
        return
    fi
    if command -v zenity >/dev/null 2>&1; then
        zenity --error --title='Eltec v6.1 tester could not start' --text="$details" \
            >/dev/null 2>&1 || true
...
{
    printf '\n[%s] Starting Eltec 406MCA ESP32 Tester v6.1\n' "$(date '+%Y-%m-%d %H:%M:%S')"
