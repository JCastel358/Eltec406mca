...
class ConnectionTests(unittest.TestCase):
    ...
    def test_v3_firmware_gets_qualified_front_end_on_connect(self):
        ...
        # unbuffered front end; connecting for 406MCA testing must restore the
        # qualified gain-2 buffered front end and verify the read-back.
        ...
        self.assertIn("FE,V19", serial_port.writes)
        self.assertIn("FE?", serial_port.writes)
        self.assertLess(
            serial_port.writes.index("FE,V19"), serial_port.writes.index("FE?")
        )

    def test_legacy_v1_firmware_never_receives_fe_commands(self):
        # A standalone 406MCA rig on v1.9 already runs the qualified front end
        # natively and does not understand FE commands.
        serial_port = ScriptedSerial({"IDN?": ["ELTEC-ESP32-ADS1256,v1.9"]})
        rig = rig_for(serial_port)
        rig.connect()
        self.assertTrue(rig.connected)
        self.assertNotIn("FE,V19", serial_port.writes)
        self.assertNotIn("FE?", serial_port.writes)

    def test_v2_0_is_rejected_because_it_cannot_select_qualified_front_end(self):
        serial_port = ScriptedSerial(
            {"IDN?": ["ELTEC-ESP32-ADS1256,v2.0"]}
        )
        rig = rig_for(serial_port)
        with self.assertRaisesRegex(
            backend.UnsupportedFirmwareError, "v2.1 or newer"
        ):
            rig.connect()
        self.assertTrue(serial_port.closed)

    def test_front_end_ack_mismatch_refuses_the_connection(self):
        serial_port = ScriptedSerial(
            {
                "IDN?": ["ELTEC-ESP32-ADS1256,v3.0"],
                "FE,V19": ["OK,FE,gain=1,buf=0"],
            }
        )
        rig = rig_for(serial_port)
        with self.assertRaisesRegex(
            backend.Esp32ConnectionError, "not applied as expected"
        ):
            rig.connect()
        self.assertFalse(rig.connected)

    def test_front_end_readback_mismatch_refuses_the_connection(self):
        serial_port = ScriptedSerial(
            {
                "IDN?": ["ELTEC-ESP32-ADS1256,v3.0"],
                "FE,V19": ["OK,FE,gain=2,buf=1"],
                "FE?": ["FE,gain=1,buf=0,fs=5.000"],
            }
        )
        rig = rig_for(serial_port)
        with self.assertRaisesRegex(
            backend.Esp32ConnectionError, "read-back does not match"
        ):
            rig.connect()
        self.assertFalse(rig.connected)
...
class ScalarAndPwmTests(unittest.TestCase):
    ...
    def test_scalar_reads_and_gui_compatible_pwm_signatures(self):
        self.assertAlmostEqual(
            ...
            6.1110
            * backend.BATTERY_DIVIDER_RATIO
            / backend.LEGACY_FIRMWARE_BATTERY_DIVIDER_RATIO,
        )
        self.assertAlmostEqual(
