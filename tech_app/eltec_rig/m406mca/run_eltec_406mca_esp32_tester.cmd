@echo off
rem Launch the Windows/ESP32 406MCA (v6.1 policy) tester from any working directory.
rem Windows counterpart of run_eltec_406mca_esp32_tester.sh - same app, same
rem results folder (%USERPROFILE%\Documents\Eltec_406MCA_Test_Results\v6_1_esp32).
setlocal EnableExtensions EnableDelayedExpansion

set "SCRIPT_DIR=%~dp0"
if "%SCRIPT_DIR:~-1%"=="\" set "SCRIPT_DIR=%SCRIPT_DIR:~0,-1%"
set "APP_PATH=%SCRIPT_DIR%\eltec_406mca_esp32_tester.py"

rem --- log file (mirrors the Xubuntu launcher's ~/.local/state location) ----
set "LOG_DIR=%LOCALAPPDATA%\eltec-406mca-esp32-v6-1"
if not exist "%LOG_DIR%" mkdir "%LOG_DIR%" 2>nul
if not exist "%LOG_DIR%" set "LOG_DIR=%TEMP%\eltec-406mca-esp32-v6-1"
if not exist "%LOG_DIR%" mkdir "%LOG_DIR%" 2>nul
if not exist "%LOG_DIR%" (
    echo Eltec 406MCA ESP32 Tester v6.1: could not create a log directory. 1>&2
    exit /b 1
)
...
>>"%LOG_FILE%" 2>&1 (
    echo.
    echo [%DATE% %TIME%] Starting Eltec 406MCA ESP32 Tester v6.1
...
echo Eltec 406MCA ESP32 Tester v6.1: %MESSAGE% 1>&2
...
powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command ^
    "Add-Type -AssemblyName System.Windows.Forms; [void][System.Windows.Forms.MessageBox]::Show($env:MESSAGE + [Environment]::NewLine + [Environment]::NewLine + 'Details: ' + $env:LOG_FILE, 'Eltec 406MCA tester could not start', 'OK', 'Error')" >nul 2>&1
goto :eof
