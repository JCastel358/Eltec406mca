...
# Keep these preflight limits aligned with the production v6.1 rig without
# importing the Tk application (and therefore Tk/Matplotlib) into this CLI.
# Battery monitoring is DISABLED on the unified test rig (no battery on the
# AIN7 divider since the 2026-08-12 fixture rewiring; mirrors the production
# GUI's BATTERY_MONITORING_ENABLED flag). Set back to True on a legacy
# standalone 406MCA rig with the 6 V SLA still wired to AIN7.
...
def _validate_preflight(battery_v: float | None, offset_v: float) -> None:
    ...
    # measurable on AIN7 on the unified-rig fixture) - only the offset half
...
def capture_calibration(
    ...
    *,
    ...
    try:
        ...
        # measurable on AIN7 on the unified-rig fixture).
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Collect and summarize v6.1 peak-delta calibration evidence from "
