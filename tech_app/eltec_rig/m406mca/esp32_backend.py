...
# board that supports the runtime front-end switch (v2.1+) the connect
...
INCOMPATIBLE_V2_FRONT_END_VERSION = (2, 0, 0)
QUALIFIED_FRONT_END_COMMAND = "FE,V19"
QUALIFIED_FRONT_END_GAIN = 2
QUALIFIED_FRONT_END_BUFFERED = True
...
class Esp32Rig:
    ...
    def _open_and_handshake(self, port_name: str) -> None:
        ...
        elif identity.version >= INCOMPATIBLE_V2_FRONT_END_VERSION:
            raise UnsupportedFirmwareError(
                "Eltec firmware v2.0 boots with the 405 M22 gain-1/unbuffered "
                "front end but cannot switch back to the qualified 406 MCA "
                "front end. Flash unified firmware v2.1 or newer (v3.0 is "
                "current), or use a legacy standalone v1.9 406 MCA board."
            )

    def _select_qualified_front_end(self) -> None:
        """Put a v2.1+ board back on the 406MCA-qualified ADS1256 front end.

        Sends ``FE,V19`` (gain 2, input buffer ON — the front end every 406MCA
        threshold was qualified on) and verifies both the command acknowledge
        and an independent ``FE?`` read-back. Any mismatch is a hard protocol
        error: measuring a 406MCA on the gain-1 unbuffered front end would
        ...
        if expected not in ack:
            raise Esp32ProtocolError(
                f"{QUALIFIED_FRONT_END_COMMAND} was not applied as expected: "
                f"{ack!r} (need {expected})."
            )
