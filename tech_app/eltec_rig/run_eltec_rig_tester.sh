#!/usr/bin/env bash
# Launch the unified Eltec Test Rig sensor-version selector from any working
# directory (Xubuntu). Windows counterpart: run_eltec_rig_tester.cmd.
...
LOG_DIR="$STATE_ROOT/eltec-rig"
if ! mkdir -p -- "$LOG_DIR" 2>/dev/null; then
    LOG_DIR="${TMPDIR:-/tmp}/eltec-rig-${UID}"
    mkdir -p -- "$LOG_DIR" || {
        printf 'Eltec Test Rig: could not create a log directory.\n' >&2
        exit 1
    }
fi
...
show_error() {
    ...
    printf 'Eltec Test Rig: %s\nDetails: %s\n' "$message" "$LOG_FILE" >&2

    if command -v notify-send >/dev/null 2>&1 \
        && notify-send --urgency=critical --app-name='Eltec Test Rig' \
            'Eltec Test Rig could not start' "$details" >/dev/null 2>&1; then
        return
    fi
    if command -v zenity >/dev/null 2>&1; then
        zenity --error --title='Eltec Test Rig could not start' --text="$details" \
            >/dev/null 2>&1 || true
