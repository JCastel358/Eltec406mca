...
def stable_stream(sample_count: int = 8002) -> list[backend.StreamSample]:
    """Return 1 Hz square-wave samples with rising edges at 1, 1001, ..."""

    samples: list[backend.StreamSample] = []
    for index in range(sample_count):
        phase = index % 1000
        sync = 1 if 1 <= phase <= 500 else 0
...
class FakeRig:
    ...
    def connect(self) -> None:
        ...
        self.identity = backend.FirmwareIdentity(
            "ELTEC-ESP32-ADS1256,v2.0.2", (2, 0, 2), True
        )
...
class ParserTests(unittest.TestCase):
    def test_capture_parser_exposes_requested_interface_and_defaults(self):
        ...
        self.assertEqual(calibration.DEFAULT_OUTPUT_DIR.parent.name, "405m22_esp32")
