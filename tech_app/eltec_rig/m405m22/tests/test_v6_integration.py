...
def synthetic_noise_waveform(
    *,
    ...
    high for the first and last window quarters and low in between: runs are
    longer than the 20-sample band-limiting boxcar (full pk-pk survives) and
    the pattern is symmetric about the window center, so the per-window
    least-squares detrend fits a zero slope and leaves the pk-pk exact.
    """
...
class NoiseWorkflowTests(unittest.TestCase):
    ...
    def test_noise_report_csv_round_trip_for_a_failing_capture(self):
        ...
        self.assertAlmostEqual(float(row["noise_worst_pp_mv"]), 2.000, delta=0.02)
    ...
    def test_quiet_sensor_passes_and_records_noise_stats(self):
        ...
        self.assertAlmostEqual(report.worst_pp_mv, 0.020, delta=0.001)
