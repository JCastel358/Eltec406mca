#!/usr/bin/env bash
# Install Eltec Test Rig XFCE desktop and per-user Applications-menu launchers.
...
ENTRY_NAME='com.eltec.test-rig.desktop'
...
TMP_ENTRY=$(mktemp "${TMPDIR:-/tmp}/eltec-rig-launcher.XXXXXX.desktop")
...

{
    ...
    printf '%s\n' 'Type=Application'
    printf '%s\n' 'Name=Eltec Test Rig'
    printf '%s\n' 'GenericName=Eltec Sensor Test Rig'
    printf '%s\n' 'Comment=Choose the sensor version (405 M22, 406 MCA, ...) and test on the ESP32/ADS1256 rig'
    printf 'Exec=%s\n' "$EXEC_VALUE"
    printf 'Icon=%s\n' "$ICON_VALUE"
    printf '%s\n' 'Terminal=false'
    printf '%s\n' 'Categories=Utility;'
...
printf 'Installed the Eltec Test Rig launcher:\n'
