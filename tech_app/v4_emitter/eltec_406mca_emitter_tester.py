"""
Eltec 406MCA emitter tester v4 - guided workflow with a full visual overhaul.

The measurement engine, wiring, and guided flow are identical to v3; v4 is a
presentation-layer rebuild styled after eltecinstruments.com:
    ...
    - Dark navy oscilloscope panel with grid + glow traces (site "dark section"
      look), JetBrains Mono / Consolas for all technical readouts
...
BATTERY_READ_SAMPLES = 12          # high-res command-response reads; median of 12 is stable
BATTERY_READ_DELAY_S = 0.005
# Reuse the load-step battery check instead of re-reading before the capture
# when the reading is healthy and at most this old (the 9V sags slowly).
BATTERY_REUSE_WINDOW_S = 30.0

# Hardware plausibility guards ("is everything actually plugged in?").
# A floating LabJack input reads arbitrary voltages, which can look like a
...
BATTERY_FAULT_MIN_V = 3.0    # below this: battery missing / AIN1 divider not wired
BATTERY_FAULT_MAX_V = 10.5   # above this: not a 9V reading (floating input)
...
SYNC_CHECK_CYCLES = 3        # ~0.3 s pre-flight peek at AIN2 after PWM starts
...
# Battery level as displayed by the header gauge: full at ~9.5 V, empty at the block level.
BATTERY_GAUGE_FULL_V = 9.5
...
# --------------------------------------------------------------------------- #
# Fast capture (v4). The v3 stopping rule cannot even check stability before
# settle 5 + 2 x 20-cycle windows = 45 cycles (4.5 s at 10 Hz). v4 adds a
...
# --------------------------------------------------------------------------- #
# v4 theme - palette sampled from eltecinstruments.com
# --------------------------------------------------------------------------- #
...
def load_private_fonts() -> None:
    ...
    The v4 look prefers Poppins / Manrope / JetBrains Mono (the faces used on
...
CSV_FIELDS = [
    ...
    "noise_rms_mv",
    "snr_db",
    # v4 capture telemetry + fast-path validation log.
    "capture_mode",
    "capture_cycles",
    "capture_seconds",
    "fast_stop_cycle",
...
def results_root_dir() -> Path:
    ...
    return Path.home() / "Documents" / "Eltec_406MCA_Test_Results" / "v4_emitter"
...
class EmitterLabJackT7(LabJackT7):
    ...
    def read_waveform_stream_decided(
        ...
        *,
        ...
        """Stream AIN0/AIN2 with mode-dependent stopping.
        ...
        scan_list = ljm.namesToAddresses(len(scan_names), scan_names)[0]
        scans_per_read = max(20, int(sample_rate_hz / 10.0))
        target_scans = int(math.ceil((DEFAULT_MAX_CAPTURE_CYCLES / expected_frequency_hz) * sample_rate_hz))
        ...
        try:
            ...
            while len(waveform) < target_scans:
                ...
                sync.extend(arr[:, 0].tolist())
                waveform.extend(arr[:, -1].tolist())
                cycles_seen = len(waveform) / actual_scan_rate * expected_frequency_hz
        ...
        waveform_np = np.asarray(waveform[:target_scans], dtype=float)
        sync_np = np.asarray(sync[:target_scans], dtype=float)
...
def battery_state_for(battery_v: float | None) -> str:
    ...
    'fault' means the number cannot be a real 9V battery through the divider
    (missing battery / AIN1 not wired / floating input) - testing is blocked.
    """
...
def battery_gauge_fraction(battery_v: float | None) -> float:
    """Map the 9V reading to a 0..1 fill for the header battery gauge."""
    if battery_v is None:
        return 0.0
...
class CaptureReport:
    ...
    data_source: str = ""               # "hardware" | "simulator"
...
# --------------------------------------------------------------------------- #
# v4 UI toolkit - colors, easing, animation engine
# --------------------------------------------------------------------------- #
...
class ToggleSwitch(tk.Frame):
    ...
    def __init__(self, parent: tk.Widget, text: str, variable: tk.BooleanVar, command=None, bg: str = PAGE_BG, font=("Segoe UI", 12)) -> None:
...
class BatteryPill(tk.Canvas):
    ...
    def _redraw(self) -> None:
        ...
        self.create_text(bx + bw + S(13), self._ph / 2, anchor="w", text=self._text, fill="#ffffff", font=("Segoe UI", 11, "bold"))
    ...
    def redraw(self) -> None:
        ...
        if waveform.size < 2:
            self.create_text(
                ...
                fill="#3d4f78", font=("Consolas", 11, "bold"),
            )
            mid_y = height / 2 + S(18)
            ...
            self.create_text(width - self._pad_x, S(12), anchor="e", text=f"{lo:+.4f} V  …  {hi:+.4f} V", fill="#8ea6d4", font=("Consolas", 10))
        self._chip(self._pad_x, S(8), "AIN0 · SENSOR", TRACE_CORE)
        if waveform.size >= 2 and sync.size == waveform.size:
            self._chip(self._pad_x, int(height * 0.72) - S(24), "AIN2 · PWM SYNC", SYNC_CORE)


# --------------------------------------------------------------------------- #
# Guided emitter tester UI (v4)
# --------------------------------------------------------------------------- #
class EmitterTesterApp(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            pass
        self.title("Eltec 406MCA Emitter Tester v4")
        self.minsize(S(1100), S(740))

        self.animator = Animator(self)
        load_private_fonts()
        self.FONT_DISPLAY = pick_font_family(self, ["Poppins SemiBold", "Poppins", "Manrope", "Segoe UI Semibold", "Segoe UI"], "Segoe UI")
        self.FONT_BODY = pick_font_family(self, ["Manrope", "Roboto", "Segoe UI"], "Segoe UI")
        self.FONT_MONO = pick_font_family(self, ["JetBrains Mono", "Cascadia Code", "Cascadia Mono", "Consolas"], "Consolas")
    ...
    def _on_mousewheel(self, event: tk.Event) -> None:
        ...
        try:
            ...
            if first <= 0.0 and last >= 1.0:
                return
            self.step_scroll.yview_scroll(-3 * int(event.delta / 120), "units")
        except tk.TclError:
            pass
    ...
    def _redraw_header(self, _event=None) -> None:
        ...
        self.header.create_text(chip_x + S(20), S(26), text="V4", fill="#ffffff", font=self.fm(11, "bold"), tags="static")
        if self.simulator_var.get():
    ...
    def _build_battery_banner(self) -> None:
        ...
        if self.battery_state == "fault":
            message = (f"Battery reads{volts}, which is not a valid 9V level — the battery or the AIN1 "
    ...
    def _build_wave_canvas(self, row: int, live: bool) -> None:
        ...
        tk.Label(wrapper, text="LIVE SIGNAL  ·  AIN0 SENSOR + AIN2 PWM SYNC", bg=PAGE_BG, fg=MUTED_FG, font=self.fm(10, "bold")).grid(row=0, column=1, sticky="w")
    ...
    def update_navigation_state(self) -> None:
        ...
        if self.step == self.SETUP_STEP:
            ...
            if self.battery_state == "fault":
                ...
                measure_text = "Change battery to test"
            else:
    ...
    def _refresh_battery_pill(self) -> None:
        if self.battery_v is None:
            ...
            text = f"CHANGE BATTERY  {self.battery_v:.1f} V"
    ...
    def run_measurement(self, _event: tk.Event | None = None) -> None:
        ...
        if self.battery_state in ("low", "fault"):
            volts = "" if self.battery_v is None else f" ({self.battery_v:.2f} V)"
            if self.battery_state == "fault":
                self.status_var.set(f"Battery reading is not valid{volts}. Check the battery clip and AIN1 wiring, then press “Re-check battery”.")
            else:
                self.status_var.set(f"Battery too low{volts}. Change the 9V battery and press “Re-check battery”.")
            self.refresh_battery()
            return
        ...
        with self.hardware_lock:
            ...
            if battery_state_for(battery_v) == "fault":
                raise HardwareNotReadyError(
                    f"The battery input reads {battery_v:.2f} V, which is not a valid 9V level. "
                    "The battery or the AIN1 100k/100k divider is probably not connected - "
            ...
            push(lambda: self.set_measure_status(token, "Emitter PWM on. Checking the AIN2 sync signal..."))
            try:
                # Pre-flight: the PWM loops back into AIN2. If no square wave
                # shows up within a few cycles, the gate drive or the loopback
                # wire is missing - abort instead of capturing garbage.
                ...
                if sync_span < SYNC_MIN_SPAN_V:
                    raise HardwareNotReadyError(
                        f"PWM sync not detected: AIN2 shows only {sync_span:.2f} V of swing. Check the "
                        f"{EMITTER_PWM_CHANNEL} -> MOSFET gate drive and the AIN2 loopback wire, then press Measure again."
                    )
                if show_live:
    ...
    def _friendly_hardware_error(text: str) -> str:
        upper = text.upper()
        if "1230" in text or "CLAIMED" in upper:
            return "The T7 is claimed by another LabJack program. Close LJStreamM/Kipling and try again."
        if "NOT_FOUND" in upper or "NOT FOUND" in upper or "NO DEVICES" in upper or "LJM LIBRARY" in upper:
            return ("No LabJack T7 detected. Plug in the LabJack USB cable and rig power, then try again. "
    ...
    def startup_probe(self) -> None:
        ...
        # auto-enable it. v3 silently switched to the simulator when no T7 was
        # found, which let a technician run a "test" against synthetic numbers
        # (battery pinned at 8.8 V, plausible-looking results) without
        # noticing that nothing was plugged in.
        if not ok and not self.simulator_var.get() and message.startswith("No T7 detected"):
            message = ("No LabJack T7 detected - plug in the USB cable and rig wiring before testing. "
                       "(Training without hardware: Advanced options -> Simulator mode.)")
