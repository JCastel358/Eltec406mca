...
The current firmware source is `Arduino/Eltec/Eltec.ino` **v2.1**
(2026-08-12), required by the 405 M22 build:
...
The 405 M22 work is local and uncommitted. Current expected status: the
new/untracked `tech_app/405m22_esp32/` directory,
`Arduino/Eltec/ESP32_ADS1256_Wiring_v2_0.md`, and the new
`Arduino/Eltec/versions/` archive; modified `Arduino/Eltec/Eltec.ino`
(v2.0), modified `Arduino/Eltec/esp32_rig_readout.py` and
`Arduino/Eltec/live_waveform.py` (`--freq` support; bulk-read lag fix + lag
readout in live_waveform), and this modified `status.md`. There is no
tracked diff under `tech_app/v6_esp32/` or `tech_app/v6_1_esp32/`.

**Commit soon.** v1.9 had to be reconstructed precisely because the live
sketch was edited twice without a commit in between; `versions/` is a
safety net, not a substitute for committing each firmware bump.
