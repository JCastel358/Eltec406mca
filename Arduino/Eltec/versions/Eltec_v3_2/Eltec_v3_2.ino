/*
  ...
  ------------------------------------------------------------------
    IDN?             -> ELTEC-ESP32-ADS1256,v3.2
                        (v3.2 = runtime-selectable emitter duty cycle via
                         ...
                         sending PIN,25. The host apps send PIN,33.
  ...
  ----------------------------------------------------------------
    ...
    D33 (GPIO33)  -> MOSFET driver module PWM/TRIG input (emitter drive,
...
#include <SPI.h>
#include "driver/gpio.h"  // gpio_hold_dis / gpio_reset_pin (GPIO33 is RTC-capable)

// ---------------------------------------------------------------- pins ----
static const int PIN_CS = 5;
static const int PIN_DRDY = 4;
// Default emitter-gate pin. Moved to D33 (GPIO33) on 2026-08-25; the
// perf-board wire moves with it (it was on D25 from 2026-07-13). GPIO33 is
// RTC-capable (hold latch released at attach) but NOT a DAC pin, unlike
// GPIO25/26. Changeable at runtime with PIN,<n>. Not persisted.
static int pinGate = 33;
