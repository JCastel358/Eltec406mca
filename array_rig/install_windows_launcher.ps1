<#
.SYNOPSIS
    Install Desktop and Start Menu shortcuts for the Eltec Array Rig.
...
.DESCRIPTION
    ...
    rights are needed. Both shortcuts point at run_eltec_array_tester.cmd in
...
$Launcher = Join-Path $ScriptDir 'run_eltec_array_tester.cmd'
$AppPath = Join-Path $ScriptDir 'eltec_array_tester.py'
...
try {
    foreach ($link in @($DesktopLink, $StartMenuLink)) {
        ...
        $shortcut.Description = 'Eltec array test rig - choose the detector model (40623, ...) and test fifty parts at once on the DAQ array rig'
...
""
"Launcher : $Launcher"
"Log file : $env:LOCALAPPDATA\eltec-array-rig\launcher.log"
"Results  : $([Environment]::GetFolderPath('MyDocuments'))\Eltec_40623_Test_Results\40623_array_daq (40623)"
