@echo off
rem Launch the Eltec Array Rig (50-position DAQ) sensor-version selector from any working
rem directory. Windows counterpart of run_eltec_array_tester.sh - same app; each
rem sensor version keeps its own results folder (see README.md).
setlocal EnableExtensions EnableDelayedExpansion

set "SCRIPT_DIR=%~dp0"
if "%SCRIPT_DIR:~-1%"=="\" set "SCRIPT_DIR=%SCRIPT_DIR:~0,-1%"
set "APP_PATH=%SCRIPT_DIR%\eltec_array_tester.py"

rem --- log file (mirrors the Xubuntu launcher's ~/.local/state location) ----
set "LOG_DIR=%LOCALAPPDATA%\eltec-array-rig"
if not exist "%LOG_DIR%" mkdir "%LOG_DIR%" 2>nul
if not exist "%LOG_DIR%" set "LOG_DIR=%TEMP%\eltec-array-rig"
if not exist "%LOG_DIR%" mkdir "%LOG_DIR%" 2>nul
if not exist "%LOG_DIR%" (
    echo Eltec Array Rig: could not create a log directory. 1>&2
    exit /b 1
)
...
>>"%LOG_FILE%" 2>&1 (
    echo.
    echo [%DATE% %TIME%] Starting Eltec Array Rig
...
echo Eltec Array Rig: %MESSAGE% 1>&2
...
powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command ^
    "Add-Type -AssemblyName System.Windows.Forms; [void][System.Windows.Forms.MessageBox]::Show($env:MESSAGE + [Environment]::NewLine + [Environment]::NewLine + 'Details: ' + $env:LOG_FILE, 'Eltec Array Rig could not start', 'OK', 'Error')" >nul 2>&1
goto :eof
