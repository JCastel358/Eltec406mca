#!/usr/bin/env bash
# Install Eltec 40623 Array Tester XFCE desktop and per-user Applications-menu launchers.
...
LAUNCHER="$SCRIPT_DIR/run_eltec_40623_array_tester.sh"
APP_PATH="$SCRIPT_DIR/eltec_40623_array_tester.py"
ICON_PATH="$SCRIPT_DIR/assets/eltec_desktop_icon.png"
ENTRY_NAME='com.eltec.40623-array-tester.desktop'
...
DESKTOP_ENTRY="$DESKTOP_DIR/Eltec 40623 Array Tester.desktop"
...
TMP_ENTRY=$(mktemp "${TMPDIR:-/tmp}/eltec-40623-array-launcher.XXXXXX.desktop")
...

{
    ...
    printf '%s\n' 'Version=1.0'
    printf '%s\n' 'Type=Application'
    printf '%s\n' 'Name=Eltec 40623 Array Tester'
    printf '%s\n' 'GenericName=Eltec 40623 Array Tester (50 positions)'
    printf '%s\n' 'Comment=TP120 offset and noise on fifty 40623 detectors at once (DAQ array rig)'
    printf 'Exec=%s\n' "$EXEC_VALUE"
    printf 'Icon=%s\n' "$ICON_VALUE"
    printf '%s\n' 'Terminal=false'
    printf '%s\n' 'Categories=Utility;'
...
printf 'Installed the Eltec 40623 Array Tester launcher:\n'
