...
class OperatorGuiTests(unittest.TestCase):
    ...
    def test_offset_check_shows_high_low_and_dead_without_fifty_inputs(self):
        ...
        grid = self.window.grid
        self.assertEqual(grid.itemcget(grid._items["1-1"]["headline"], "text"), "PASS")
        self.assertEqual(grid.itemcget(grid._items["2-4"]["headline"], "text"), "FAIL")
        self.assertEqual(grid.itemcget(grid._items["1-1"]["detail"], "state"), "hidden")
        self.window.show_more_button.invoke()
    ...
    def test_noise_autosaves_demo_red_green_low_then_next_tray_resets(self):
        ...
        grid = self.window.grid
        self.assertEqual(grid.itemcget(grid._items["3-6"]["headline"], "text"), "FAIL")
        self.assertEqual(grid.itemcget(grid._items["1-1"]["headline"], "text"), "PASS")
        self.assertEqual(grid.itemcget(grid._items["1-1"]["number"], "state"), "hidden")
        saved_report = controller.state.report
        self.window.show_more_button.invoke()
        self.assertIn("µV", grid.itemcget(grid._items["1-1"]["headline"], "text"))
    ...
    def test_unset_production_noise_limits_render_no_limit_not_pass(self):
        ...
        c.plan = tester.CapturePlan(capture_seconds=20, stabilisation_s=0, quiet_min_s=1, quiet_max_s=4)
        self._noise()
        self.assertTrue(c.state.saved, self.window.status_var.get())
        self.assertEqual(self._detail("1-1"), "NO LIMIT")
        self.assertEqual(self.window.grid.itemcget(self.window.grid._items["1-1"]["headline"], "text"), "NO LIMIT")
