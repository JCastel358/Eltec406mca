@echo off
rem Launch the 40623 array tester (50-position DAQ rig) from any working directory.
rem Windows counterpart of run_eltec_40623_array_tester.sh - same app, same
rem results folder (%USERPROFILE%\Documents\Eltec_40623_Test_Results\40623_array_daq).
setlocal EnableExtensions EnableDelayedExpansion

set "SCRIPT_DIR=%~dp0"
if "%SCRIPT_DIR:~-1%"=="\" set "SCRIPT_DIR=%SCRIPT_DIR:~0,-1%"
set "APP_PATH=%SCRIPT_DIR%\eltec_40623_array_tester.py"

rem --- log file (mirrors the Xubuntu launcher's ~/.local/state location) ----
set "LOG_DIR=%LOCALAPPDATA%\eltec-40623-array"
if not exist "%LOG_DIR%" mkdir "%LOG_DIR%" 2>nul
if not exist "%LOG_DIR%" set "LOG_DIR=%TEMP%\eltec-40623-array"
if not exist "%LOG_DIR%" mkdir "%LOG_DIR%" 2>nul
if not exist "%LOG_DIR%" (
    echo Eltec 40623 Array Tester: could not create a log directory. 1>&2
    exit /b 1
)
...
>>"%LOG_FILE%" 2>&1 (
    echo.
    echo [%DATE% %TIME%] Starting Eltec 40623 Array Tester
...
echo Eltec 40623 Array Tester: %MESSAGE% 1>&2
...
powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command ^
    "Add-Type -AssemblyName System.Windows.Forms; [void][System.Windows.Forms.MessageBox]::Show($env:MESSAGE + [Environment]::NewLine + [Environment]::NewLine + 'Details: ' + $env:LOG_FILE, 'Eltec 40623 array tester could not start', 'OK', 'Error')" >nul 2>&1
goto :eof
