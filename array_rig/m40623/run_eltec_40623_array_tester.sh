#!/usr/bin/env bash
# Launch the 40623 array tester (50-position DAQ rig) from any working directory.
...
LOG_DIR="$STATE_ROOT/eltec-40623-array"
if ! mkdir -p -- "$LOG_DIR" 2>/dev/null; then
    LOG_DIR="${TMPDIR:-/tmp}/eltec-40623-array-${UID}"
    mkdir -p -- "$LOG_DIR" || {
        printf 'Eltec 40623 Array Tester: could not create a log directory.\n' >&2
        exit 1
    }
fi
...
show_error() {
    ...
    printf 'Eltec 40623 Array Tester: %s\nDetails: %s\n' "$message" "$LOG_FILE" >&2

    if command -v notify-send >/dev/null 2>&1 \
        && notify-send --urgency=critical --app-name='Eltec 40623 Array Tester' \
            'Eltec 40623 array tester could not start' "$details" >/dev/null 2>&1; then
        return
    fi
    if command -v zenity >/dev/null 2>&1; then
        zenity --error --title='Eltec 40623 array tester could not start' --text="$details" \
            >/dev/null 2>&1 || true
...
{
    printf '\n[%s] Starting Eltec 40623 Array Tester\n' "$(date '+%Y-%m-%d %H:%M:%S')"
