#!/usr/bin/env bash
# Install Eltec Array Rig XFCE desktop and per-user Applications-menu launchers.
...
DESKTOP_ENTRY="$DESKTOP_DIR/Eltec Array Rig.desktop"
...
TMP_ENTRY=$(mktemp "${TMPDIR:-/tmp}/eltec-array-rig-launcher.XXXXXX.desktop")
...

{
    ...
    printf '%s\n' 'Version=1.0'
    printf '%s\n' 'Type=Application'
    printf '%s\n' 'Name=Eltec Array Rig'
    printf '%s\n' 'GenericName=Eltec Array Test Rig'
    printf '%s\n' 'Comment=Choose the detector model (40623, ...) and test fifty parts at once on the DAQ array rig'
    printf 'Exec=%s\n' "$EXEC_VALUE"
    printf 'Icon=%s\n' "$ICON_VALUE"
    printf '%s\n' 'Terminal=false'
    printf '%s\n' 'Categories=Utility;'
...
printf 'Installed the Eltec Array Rig launcher:\n'
